...
2. På fanen **Konfigurationsnøgler** kan du udvide noden **Detail** og derefter markere afkrydsningsfeltet **Fordelt ordrestyring**.
3. Gå til **Detail \> Fordelt ordrestyring \> Opsætning \> DOM parametre**.
4. På fanen **Generelt** kan du angive følgende værdier:
    ...
    - **Bekræft brug af Bing Kort til DOM** – Angiv denne indstilling til **Ja**.
        ...
        > [!NOTE]
        > Du kan kun angive denne indstilling **Ja**, hvis indstillingen **Aktivér Bing Kort** på fanen **Bing Kort** på siden **Delte parametre for detail** (**Detail \> Konfiguration af hovedkontor \> Parametre \> Delte parametre for detail**) også er angivet til **Ja**, og hvis en gyldig nøgle er angivet i feltet **Bing Kort-nøgle** felt.
...
5. På fanen **Problemløser** kan du angive følgende værdier:
    ...
    - **Type af problemløser** – Vælg en værdi. To typer problemløsere frigives med Detail: **Problemløser for produktion** og **Forenklet problemløser**. For alle computere, der skal køre DOM (dvs, alle servere, der tilhører gruppen DOMBatch), skal **Problemløser for produktion** være valgt. Problemløser for produktion kræver den særlige licensnøgle, der som standard er givet i licens og installeret i produktionsmiljøer. For ikke-produktionsmiljøer skal denne licensnøgle installeres manuelt. Du kan installere licensnøglen manuelt ved at gøre følgende:
...
6. Gå tilbage **Detail \> Fordelt ordrestyring \> Opsætning \> DOM-parametre**.
...
8. DOM-funktionen understøtter definitionen af forskellige typer DOM-regler, og organisationer kan konfigurere flere regler afhængigt af deres forretningsbehov. DOM-regler kan defineres for en gruppe af lokationer eller individuelle lokationer og for en bestemt produktkategori, et produkt eller en variant. For at oprette den gruppering af lokationer, der skal anvendes for DOM-reglerne, skal du følge disse trin:
    ...
    1. Gå til **Detail \> Konfiguration af kanal \> Opfyldelsesgrupper**.
...
9. For at angive regler skal du gå til **Detail \> Fordelt ordrestyring \> Opsætning \> Administrer regler**. Følgende DOM-regler understøttes i øjeblikket:
...
10. Opfyldningsprofiler bruges til at gruppere en samling regler, juridiske enheder, salgsordreoprindelser og leveringsmåder. Hver DOM-kørsel er for en bestemt opfyldelsesprofil. På denne måde kan organisationer definere og køre et sæt regler for et sæt juridiske enheder på ordrer, der har en bestemt salgsordreoprindelse og leveringsmåde. Hvis der skal køres forskellige sæt regler for forskellige sæt salgsordreoprindelser eller leveringsmåder, kan opfyldningsprofilerne derfor defineres i overensstemmelse hermed. Benyt følgende fremgangsmåde for at konfigurere opfyldelsesprofiler:  
    ...
    1. Gå til **Detail \> Fordelt ordrestyring \> Opsætning \> Opfyldningsprofiler**.
    2. Vælg **Ny**.
...
1. Gå til **Detail \> Fordelt ordrestyring \> Batchbehandling \> Opsætning af DOM-processorjob**.
...
1. Gå til **Detail \> Fordelt ordrestyring \> Fordelt ordrestyring**.
...
I det følgende beskrives indstillingerne på ordrelinjen. For at åbne ordrelinjen skal du gå til **Detail \> Kunder \> Alle salgsordrer**.
...
1. Gå til **Detail \> Fordelt ordrestyring \> Batchbehandling \> Opsætning af job til sletning af DOM-opfyldelsesdata**. 
...
- DOM ser i øjeblikket kun på ordrer, der er oprettet ud fra detailkanaler. Salgsordrer identificeres som detailsalgsordrer, når indstillingen **Detailsalg** er angivet til **Ja**.
...
- DOM er kun tilgængelig i sky-versionen af Retail. Funktionen understøttes ikke i lokale installationer.
