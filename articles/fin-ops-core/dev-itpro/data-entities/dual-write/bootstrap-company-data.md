...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="8eaf0-103">Ofte stillede spørgsmål om bootstraping med firmadata</span><span class="sxs-lookup"><span data-stu-id="8eaf0-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="8eaf0-104">Hvorfor har jeg brug for bootstrapping?</span><span class="sxs-lookup"><span data-stu-id="8eaf0-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="8eaf0-105">Du har muligvis en eksisterende Common Data Service eller anden Dynamics 365-app-forekomst med forretningsdata, og du vil aktivere forbindelse med dobbeltskrivning mod den.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="8eaf0-106">I så fald skal du bootstrappe Common Data Service eller andre programdata fra Dynamics 365 med firmaoplysninger, før forbindelse med dobbeltskrivning aktiveres.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="8eaf0-107">Hvornår bør jeg bruge bootstrapping?</span><span class="sxs-lookup"><span data-stu-id="8eaf0-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="8eaf0-108">Du bør bruge bootstrapping, før du aktiverer enhedstilknytninger for dobbeltskrivning (under trin #5).</span><span class="sxs-lookup"><span data-stu-id="8eaf0-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="8eaf0-109">Hvis du vil konfigurere forbindelsen med dobbeltskrivning mellem forekomster af din Finance and Operations-app og Common Data Service- eller anden Dynamics 365-app, skal du logge på Finance and Operations-appen som administrator.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="8eaf0-110">Gå til modulet **Datastyring**, og klik på knappen **Dobbeltskrivning-**.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="8eaf0-111">Derved startes **Data Integratoren**.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="8eaf0-112">Opret forbindelsen med dobbeltskrivning for et eller flere firmaer.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="8eaf0-113">![Opret forbindelsen med dobbeltskrivning](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="8eaf0-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="8eaf0-114">Aktiver enhedstilknytning **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="8eaf0-115">Derved synkroniseres firmaer fra Finance and Operations-appen til Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="8eaf0-116">![Aktiver enhedstilknytningen](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="8eaf0-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="8eaf0-117">Kør eksempelkoden til bootstrapping på Common Data Service eller en anden Dynamics 365-appforekomst.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="8eaf0-118">Når bootstrapping er fuldført, og systemet er klar til live synkronisering, skal du aktivere enhedstilknytningerne.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="8eaf0-119">Aktivering af enhedstilknytninger udløser den indledende datasynkronisering for de aktiverede enhedstilknytninger.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="8eaf0-120">De data, der svarer til de firmaer, som er valgt på dobbeltskrivningsforbindelsen, synkroniseres mellem Finance and Operations-appen og Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="8eaf0-121">Hvordan bruges kodeeksemplet?</span><span class="sxs-lookup"><span data-stu-id="8eaf0-121">How to I use the code sample?</span></span>
<span data-ttu-id="8eaf0-122">Kodeeksemplet er et C#-program, du kan indlæse i Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="8eaf0-123">Det tager NuGet-pakkeafhængigheder på Common Data Service-SDK, som du kan opdatere via standard Visual Studio-værktøjer.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="8eaf0-124">Når du har udpakket og åbnet løsningen i Visual Studio og gendannet NuGet-pakkerne, skal du søge efter **OPGAVE** i koden.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="8eaf0-125">Hver beslutning, du skal træffe, hvis du vil bootstrappe firmaoplysninger, noteres af **OPGAVE** med kodeeksemplet for en vedtaget implementering.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="8eaf0-126">Kodeeksemplet viser kun en af de mange måder, du kan kategorisere enhedsposter efter firma på.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="8eaf0-127">Ved at ændre logikken i sektionerne **OPGAVE**, kan du oprette din egen brugerdefineret kategorisering.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="8eaf0-128">Hvad kan jeg forvente?</span><span class="sxs-lookup"><span data-stu-id="8eaf0-128">What should I expect?</span></span>
<span data-ttu-id="8eaf0-129">Eksempelprogrammet giver dig som standard mulighed for at angive en ordbog over kodetilknytninger for virksomhedsenhed-til-firma.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="8eaf0-130">Alle de enheder, du bootstrapper med et **OwningBusinessUnit**-felt, angives automatisk til at bruge det angivne firma.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="8eaf0-131">En enhed uden et **OwningBusinessUnit**-felt, såsom et produkt, vil angive firmaet ud fra tilknytningen med en tom værdi for virksomhedsenhed.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="8eaf0-132">Konsolprogrammet forventer ét parameter, enten **–simuler** eller **–anvend**.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="8eaf0-133">Hvis du bruger kommandolinjeparametret **–simuler** vil ingen data blive opdateret.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="8eaf0-134">Der genereres alene **simulering<entityname>.csv**-filer i samme mappe som værktøjet; et for hver enhed, der ville være blevet opdateret.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="8eaf0-135">Du kan gennemse disse filer gentagne gang, mens du arbejder, for at sikre dig, at koden opdaterer firmaværdierne som forventet.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="8eaf0-136">Når du er færdig med de simulerede opdateringer, skal du bruge parametret **–anvend**.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="8eaf0-137">Derved opdateres alle poster, der aktuelt har en forkert firmaværdi, i batcher af 1000 poster ad gangen (som standard).</span><span class="sxs-lookup"><span data-stu-id="8eaf0-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="8eaf0-138">Koden er idempotent som angivet, hvilket betyder, at du kan køre den igen, og kun de firmaer, der er tildelt forkert, vil blive opdateret.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="8eaf0-139">Når der køres med koden **–anvend**, vil koden generere CSV-filer med de ændringer, der er foretaget, hvilket vil blive benævnt **anvendt_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="8eaf0-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
