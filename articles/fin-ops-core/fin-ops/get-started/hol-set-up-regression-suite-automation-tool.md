...
- <span data-ttu-id="e6d3e-107">Forstå den opsætning af installation og forudsætning , der kræves til de forskellige programmer, som understøtter Regression Suite Automation Tool (RSAT).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-107">Understand the installation and prerequisite setup that are required for the various applications that support Regression suite automation tool (RSAT).</span></span>
...
## <a name="configure-azure-devops"></a><span data-ttu-id="e6d3e-123">Konfigurer Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="e6d3e-123">Configure Azure DevOps</span></span>
...
<span data-ttu-id="e6d3e-173">Til dit LCS-projekt kan du bruge et eksisterende kundeimplementerings- eller partnerprojekt.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-173">For your LCS project, you can use an existing customer implementation or partner project.</span></span>
...
8. <span data-ttu-id="e6d3e-270">Hvis du vil gemme opgaveregistreringen lokalt og derefter overføre den til BPM via LCS, skal du følge disse trin:</span><span class="sxs-lookup"><span data-stu-id="e6d3e-270">If you want to save the task recording locally and then upload it to BPM through LCS, follow these steps:</span></span>
    ...
    3. <span data-ttu-id="e6d3e-275">Gå til BPM-biblioteket **RSAT**, og vælg den forretningsproces, hvor opgaveregistreringen skal gemmes.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-275">Go to the **RSAT** BPM library, and select the business process to save the task recording against.</span></span>
...
## <a name="get-started-with-rsat"></a><span data-ttu-id="e6d3e-352">Kom i gang med RSAT</span><span class="sxs-lookup"><span data-stu-id="e6d3e-352">Get started with RSAT</span></span>
...
### <a name="load-and-run-test-cases"></a><span data-ttu-id="e6d3e-418">Indlæse og køre testsager</span><span class="sxs-lookup"><span data-stu-id="e6d3e-418">Load and run test cases</span></span>
...
4. <span data-ttu-id="e6d3e-431">Hvis du vil kontrollere, at automatiseringsfilerne og parameterfilerne gemmes i Azure DevOps, skal du gå til Azure DevOps-projektet, vælge **Tavler \> Workflowopgaver** og vælge testsagen **Opret et nyt produkt**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-431">To verify that the automation files and parameter files are saved to Azure DevOps, go to the Azure DevOps project, select **Boards \> Work Items**, and select the **Create a new product** test case.</span></span> <span data-ttu-id="e6d3e-432">Under fanen **Vedhæftede filer** skulle du få vist fire filer:</span><span class="sxs-lookup"><span data-stu-id="e6d3e-432">On the **Attachments** tab, you should see four files:</span></span>
    ...
    - <span data-ttu-id="e6d3e-433">**.cs** – C\# automatiseringsfil</span><span class="sxs-lookup"><span data-stu-id="e6d3e-433">**.cs** – C\# automation file</span></span>
...
2. <span data-ttu-id="e6d3e-479">Vælg testsagen, og vælg derefter **Rediger** for at åbne Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-479">Select the test case, and then select **Edit** to open the Excel parameter file.</span></span>

    <span data-ttu-id="e6d3e-480">Bemærk, at værdien i feltet **Produktnummer** i **EcoResProductCreate**-arket er hard-coded.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-480">On the **EcoResProductCreate** sheet, notice that the value of the **Product number** field is hard-coded.</span></span> <span data-ttu-id="e6d3e-481">Du skal opdatere dette felt til et nyt produktnummer, før du kører testsagen igen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-481">You must update this field to a new product number before you run the test case again.</span></span>

3. <span data-ttu-id="e6d3e-482">Hvis du vil generere et entydigt produktnummer for hver kørsel uden at skulle genåbne Excel-parameterfilen og manuelt opdatere produktnummeret hver gang, skal du bruge følgende Excel-formel.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-482">To generate a unique product number for each run without having to reopen the Excel parameter file and manually update the product number every time, use the following Excel formula.</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="e6d3e-483">Udover fanen **Generelt** indeholder Excel-parameterfilen en datafane for hver formularside, som testsagen besøger.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-483">In addition to the **General** tab, the Excel parameter file contains a data tab for every form page the test case visits.</span></span>

    ![Feltet Produktnummer](./media/setup_rsa_tool_81.png)

4. <span data-ttu-id="e6d3e-485">Vælg **Gem**, og luk derefter Excel-projektmappen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-485">Select **Save**, and then close the Excel workbook.</span></span>
5. <span data-ttu-id="e6d3e-486">Vælg **Overfør** for at gemme Excel-parameterfilen i Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-486">Select **Upload** to save the Excel parameter file to Azure DevOps.</span></span>

    ![Meddelelse om vellykket overførsel](./media/setup_rsa_tool_82.png)

    > [!NOTE]
    > <span data-ttu-id="e6d3e-488">Hvis du vil køre testsager i en bestemt brugerkontekst, skal du angive brugerens e mail-id i feltet **Testbruger** under fanen **Generelt** i Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-488">To run test cases in a specific user context, enter the user's email ID in the **Test User** field on the **General** tab of the Excel parameter file.</span></span> <span data-ttu-id="e6d3e-489">I den seneste version af RSAT er layoutet i felterne i Excel-parameterfilen blevet opdateret, men konceptet forbliver uændret.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-489">In the latest version of RSAT, the layout of the fields in the Excel parameter file has been updated, but the concept remains the same.</span></span>
    >
    > ![Feltet Testbruger](./media/setup_rsa_tool_83.png)

### <a name="validate-the-results"></a><span data-ttu-id="e6d3e-491">Validere resultaterne</span><span class="sxs-lookup"><span data-stu-id="e6d3e-491">Validate the results</span></span>

- <span data-ttu-id="e6d3e-492">Vælg **Kør** for at køre testsagen igen, og kontrollér, at testsagen er bestået.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-492">Select **Run** to rerun the test case, and verify that the test case has passed.</span></span> <span data-ttu-id="e6d3e-493">Du kan få vist testresultaterne som beskrevet i afsnittet [Vise testresultaterne](#view-the-test-results).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-493">You can view the test results as described in the [View the test results](#view-the-test-results) section.</span></span>

    ![Resultatfelt er angivet til Bestået](./media/setup_rsa_tool_84.png)

### <a name="chaining-of-test-cases"></a><span data-ttu-id="e6d3e-495">Sammenkædning af testsager</span><span class="sxs-lookup"><span data-stu-id="e6d3e-495">Chaining of test cases</span></span>

<span data-ttu-id="e6d3e-496">En nøglefunktion i RSAT er sammenkædning af testsager (dvs. en test kan overføre værdier til andre test).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-496">One key feature of RSAT is the chaining of test cases (that is, the capability of a test to pass values to other tests).</span></span> <span data-ttu-id="e6d3e-497">Testsager køres efter deres definerede rækkefølge i Azure DevOps-testplanen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-497">Test cases are run according to their defined order in the Azure DevOps test plan.</span></span> <span data-ttu-id="e6d3e-498">(Denne rækkefølge kan også opdateres i selve testværktøjet). Hvis du derfor vil overføre variabler fra én testsag til en anden, er det vigtigt, at testene befinder sig i den rigtige rækkefølge.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-498">(This order can also be updated in the test tool itself.) Therefore, if you want to pass variables from one test case to another, it's very important that the tests be in the correct order.</span></span>

<span data-ttu-id="e6d3e-499">I dette afsnit skal du oprette en gemt variabel i den første testsag, oprette endnu en testsag og overføre den gemte variabel fra den første testsag til den anden testsag.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-499">In this section, you will create a saved variable in the first test case, create a second test case, and pass the saved variable from the first test case to the second test case.</span></span> <span data-ttu-id="e6d3e-500">Derefter skal du køre testsagerne som en sammenkædet testsag i RSAT.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-500">You will then run the test cases as a chained test case in RSAT.</span></span>

#### <a name="modify-an-existing-task-recording-to-create-a-saved-variable"></a><span data-ttu-id="e6d3e-501">Redigere en eksisterende opgaveregistrering for at oprette en gemt variabel</span><span class="sxs-lookup"><span data-stu-id="e6d3e-501">Modify an existing task recording to create a saved variable</span></span>

1. <span data-ttu-id="e6d3e-502">Åbn klienten.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-502">Open the client.</span></span>
2. <span data-ttu-id="e6d3e-503">Vælg knappen **Indstillinger** (tandhjulsymbolet), og vælg derefter **Arbejdsrutineoptager**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-503">Select the **Settings** button (the gear symbol), and then select **Task recorder**.</span></span>
3. <span data-ttu-id="e6d3e-504">Vælg **Rediger registrering**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-504">Select **Edit Recording**.</span></span>

    ![Knappen Rediger registrering](./media/setup_rsa_tool_85.png)

4. <span data-ttu-id="e6d3e-506">Vælg **Åbn fra Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-506">Select **Open from Lifecycle Services**.</span></span>

    ![Knappen Åbn fra Lifecycle Services](./media/setup_rsa_tool_86.png)

5. <span data-ttu-id="e6d3e-508">Vælg **Vælg biblioteket Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-508">Select **Select the Lifecycle Services library**.</span></span>

    ![Knappen Vælg biblioteket Lifecycle Services](./media/setup_rsa_tool_87.png)

    <span data-ttu-id="e6d3e-510">BPM-biblioteker indlæses fra LCS.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-510">BPM libraries are loaded from LCS.</span></span>

    ![Statusindikator](./media/setup_rsa_tool_88.png)

6. <span data-ttu-id="e6d3e-512">Når BPM-bibliotekerne er indlæst fra LCS, skal du vælge BPM-biblioteket **RSAT** og derefter den **Opret et nyt produkt**-forretningsproces, som opgaveregistrering blev knyttet til.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-512">After the BPM libraries are loaded from LCS, select the **RSAT** BPM library and the **Create a new product** business process that the task recording was associated with.</span></span> <span data-ttu-id="e6d3e-513">Vælg derefter **OK**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-513">Then select **OK**.</span></span>

    ![Valg af et BPM-bibliotek og en forretningsproces](./media/setup_rsa_tool_89.png)

7. <span data-ttu-id="e6d3e-515">Navnet på den rette opgaveregistrering angives i feltet **Registreringsnavn**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-515">The name of the appropriate task recording is entered in the **Recording name** field.</span></span> <span data-ttu-id="e6d3e-516">Vælg **Start**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-516">Select **Start**.</span></span>

    ![Navn på opgaveregistrering i feltet Registreringsnavn](./media/setup_rsa_tool_90.png)

8. <span data-ttu-id="e6d3e-518">Gå til **Administration af produktoplysninger \> Produkter**, og vælg **Nyt** for at åbne den side, hvor den oprindelige opgaveregistrering, **Opret et produkt**, blev registreret.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-518">Go to **Product information management \> Products**, and select **New** to open the page where the original task recording, **Create a product**, was recorded.</span></span>
9. <span data-ttu-id="e6d3e-519">Vælg **Indsæt trin**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-519">Select **Insert step**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="e6d3e-520">Det nye trin indsættes **efter** det trin, du har valgt i ruden.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-520">The new step is inserted **after** the step that you selected in the pane.</span></span>

    ![Knappen Indsæt trin](./media/setup_rsa_tool_91.png)

10. <span data-ttu-id="e6d3e-522">Højreklik på feltet **Produktnummer**, og vælg derefter **Arbejdsrutineoptager \> Kopiér**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-522">Right-click the **Product number** field, and then select **Task recorder \> Copy**.</span></span>

    ![Kommandoen Kopiér](./media/setup_rsa_tool_92.png)

11. <span data-ttu-id="e6d3e-524">Der tilføjes et nyt trin i ruden.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-524">A new step is added in the pane.</span></span> <span data-ttu-id="e6d3e-525">Notér værdien i feltet **Produktnummer**, da du skal bruge nummeret senere.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-525">Make a note of the value in the **Product number** field, because you will need it later.</span></span>

    ![Nyt tilføjet trin](./media/setup_rsa_tool_93.png)

12. <span data-ttu-id="e6d3e-527">Vælg **Redigeringen blev afsluttet**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-527">Select **Done editing**.</span></span>
13. <span data-ttu-id="e6d3e-528">Vælg **Gem i Lifecycle Services**, og knyt den nye opgaveregistrering til det samme BPM-bibliotek og den samme forretningsproces, som den oprindelige opgaveregistrering blev knyttet til.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-528">Select **Save to Lifecycle Services**, and associate the new task recording with the same BPM library and business process that the original task recording was associated with.</span></span> <span data-ttu-id="e6d3e-529">Du kan finde flere oplysninger i afsnittet [Oprette en opgaveregistrering og gemme den i BPM-biblioteket](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-529">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>
14. <span data-ttu-id="e6d3e-530">Gå til BPM-biblioteket, og vælg **Synkroniser testsager** for at overskrive den opgaveregistrering, der er knyttet til testsagen i Azure DevOps, som beskrevet i afsnittet [Teste synkroniseringen fra BPM til Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-530">Go to the BPM library, and select **Sync testcases** to overwrite the task recording that is attached to the test case in Azure DevOps, as described in the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>
15. <span data-ttu-id="e6d3e-531">Åbn RSAT, og vælg **Indlæs** for at genindlæse alle testsagerne i testpakken.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-531">Open RSAT, and select **Load** to reload all the test cases in the test suite.</span></span> <span data-ttu-id="e6d3e-532">Du skal generere automatiserings- og parameterfilerne for den relevante testsag ved at vælge testsagen og derefter vælge **Ny \> Generér testkørsels- og parameterfiler** som beskrevet i afsnittet [Indlæse og køre testsager](#load-and-run-test-cases).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-532">You must regenerate the automation and parameter files for the appropriate test case by selecting the test case and then selecting **New \> Generate Test Execution and Parameter files**, as described in the [Load and run test cases](#load-and-run-test-cases) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="e6d3e-533">Hvis Excel-parameterfilen er åben, mislykkes regenering.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-533">If the Excel parameter file was left open, regeneration will fail.</span></span> <span data-ttu-id="e6d3e-534">Derfor skal du kontrollere, at Excel-parameterfilen til testsagen er lukket, før du genererer den nye Excel-parameterfil.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-534">Therefore, make sure that the Excel parameter file for the test case is closed before you generate the new Excel parameter file.</span></span>

16. <span data-ttu-id="e6d3e-535">Vælg **Rediger** for at åbne den nye Excel-parameterfil.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-535">Select **Edit** to open the new Excel parameter file.</span></span> <span data-ttu-id="e6d3e-536">Du får vist en ny **Gemt variabel**-post i linje 9.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-536">You will see a new **Saved variable** entry on line 9.</span></span> <span data-ttu-id="e6d3e-537">Denne variabel, **{{EcoResProductCreate\_identifikation\_ProductNumber\_Copy}}**, gemmes i opgaveregistreringens XML-fil og kan bruges i efterfølgende test.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-537">This variable, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, is saved in the task recording's XML file and can be used in subsequent tests.</span></span>

    ![Gemt variabelpost](./media/setup_rsa_tool_94.png)

#### <a name="create-a-new-test-case"></a><span data-ttu-id="e6d3e-539">Oprette en ny testsag</span><span class="sxs-lookup"><span data-stu-id="e6d3e-539">Create a new test case</span></span>

1. <span data-ttu-id="e6d3e-540">Gå til BPM-biblioteket **RSAT**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-540">Go to the **RSAT** BPM library.</span></span>
2. <span data-ttu-id="e6d3e-541">Vælg eksempelprocessen **Supportforretningsprocesser**, og vælg derefter **Redigeringstilstand** i højre side.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-541">Select the **Sample Support Business Process** process, and then, on the right, select **Edit mode**.</span></span>
3. <span data-ttu-id="e6d3e-542">Rediger værdien i både feltet **Navn** og feltet **Beskrivelse** til **Frigiv et produkt**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-542">Change the value of both the **Name** field and the **Description** field to **Release a product**.</span></span> <span data-ttu-id="e6d3e-543">Vælg derefter **Gem**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-543">Then select **Save**.</span></span>

    ![Navn og beskrivelse er ændret til Frigiv et produkt](./media/setup_rsa_tool_95.png)

#### <a name="create-a-new-task-recording-that-has-a-validate-function"></a><span data-ttu-id="e6d3e-545">Oprette en ny opgaveregistrering, der har en valideringsfunktion</span><span class="sxs-lookup"><span data-stu-id="e6d3e-545">Create a new task recording that has a Validate function</span></span>

- <span data-ttu-id="e6d3e-546">Opret en opgaveregistrering for at frigive det produkt, der er oprettet tidligere, til den juridiske enhed USRT.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-546">Create a task recording to release the product that was created earlier to the USRT legal entity.</span></span> <span data-ttu-id="e6d3e-547">Du kan finde flere oplysninger i afsnittet [Oprette en opgaveregistrering og gemme den i BPM-biblioteket](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-547">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="e6d3e-548">Ved sammenkædede testsager anbefaler vi altid, at du finder eller filtrerer efter den post, du har brug for, ved at *skrive værdien i feltet manuelt*.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-548">For chained test cases, we always recommend that you find or filter for the record that you require by *manually typing the value of the field*.</span></span> <span data-ttu-id="e6d3e-549">På denne måde kan værktøjet bestemme den post, som handlingen skal udføres på, i den efterfølgende testsag.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-549">In that way, the tool can determine the record that the action must be taken against in the subsequent test case.</span></span>

    ![Ny opgaveregistrering, der har en valideringsfunktion](./media/setup_rsa_tool_96.png)

    <span data-ttu-id="e6d3e-551">Som den foregående illustration viser, når produktet blev fundet ved hjælp af Quick filter, men før du vælger **Frigiv produkter**, skal du validere værdien i feltet **Produktnummer** for at sikre, at produkt-id'et er det produkt-id, der blev oprettet tidligere.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-551">As the preceding illustration shows, after the product is found by using the Quick Filter, but before you select **Release products**, you validate the value of the **Product number** field to make sure that the product ID is the product ID that was created earlier.</span></span> <span data-ttu-id="e6d3e-552">Hvis du vil validere værdien, skal du højreklikke på feltet **Produktnummer** og derefter vælge **Arbejdsrutineoptager \> Valider \> Aktuel værdi**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-552">To validate the value, right-click the **Product number** field, and then select **Task recorder \> Validate \> Current Value**.</span></span>

    ![Validering af den aktuelle værdi](./media/setup_rsa_tool_97.png)

#### <a name="save-the-task-recording-to-bpm"></a><span data-ttu-id="e6d3e-554">Gemme opgaveregistreringen i BPM</span><span class="sxs-lookup"><span data-stu-id="e6d3e-554">Save the task recording to BPM</span></span>

1. <span data-ttu-id="e6d3e-555">Vælg **Gem i Lifecycle Services**, når opgaveregistreringen er fuldført.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-555">After the task recording is completed, select **Save to Lifecycle Services**.</span></span>

    ![Indstillinger for Gem](./media/setup_rsa_tool_98.png)

2. <span data-ttu-id="e6d3e-557">Biblioteksoplysninger indlæses fra LCS.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-557">Library information is loaded from LCS.</span></span>

    ![Statusindikator](./media/setup_rsa_tool_99.png)

3. <span data-ttu-id="e6d3e-559">Vælg det BPM-bibliotek, der skal knyttes til opgaveregistreringen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-559">Select the BPM library to associate the task recording with.</span></span> <span data-ttu-id="e6d3e-560">I dette selvstudium skal du vælge det BPM-bibliotek **RSAT**, der blev oprettet tidligere, og **Frigiv et produkt** som forretningsproces under det.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-560">For this tutorial, select the **RSAT** BPM library that was created earlier and the **Release a product** business process under it.</span></span> <span data-ttu-id="e6d3e-561">Vælg derefter **OK**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-561">Then select **OK**.</span></span>

#### <a name="sync-bpm-to-azure-devops"></a><span data-ttu-id="e6d3e-562">Synkronisere BPM til Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="e6d3e-562">Sync BPM to Azure DevOps</span></span>

1. <span data-ttu-id="e6d3e-563">Gå til BPM-biblioteket, og åbn biblioteket **RSAT**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-563">Go to the BPM library, and open the **RSAT** library.</span></span>
2. <span data-ttu-id="e6d3e-564">Vælg **VSTS-synkronisering** og derefter **Synkroniser testsager**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-564">Select **VSTS sync** and then **Sync test cases**.</span></span> <span data-ttu-id="e6d3e-565">Du kan finde flere oplysninger i afsnittet [Teste synkroniseringen fra BPM til Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-565">For more information, see the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>

    <span data-ttu-id="e6d3e-566">Når synkroniseringen er fuldført, vises den nye workflowopgave og den tilsvarende testsag for forretningsprocessen **Frigiv et produkt** i Azure DevOps ved **Tavler \> Workflowopgaver**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-566">After the synchronization is completed, the new work item and the corresponding test case for the **Release a product** business process appear in Azure DevOps at **Boards \> Work Items**.</span></span>

#### <a name="add-the-new-test-case-to-the-existing-test-suite"></a><span data-ttu-id="e6d3e-567">Føje den nye testsag til den eksisterende testpakke</span><span class="sxs-lookup"><span data-stu-id="e6d3e-567">Add the new test case to the existing test suite</span></span>

1. <span data-ttu-id="e6d3e-568">Gå til **Testplaner \> Testplaner**, og vælg planen **RSAT-testplan**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-568">Go to **Test plans \> Test plans**, and select the **RSAT Test Plan** plan.</span></span>
2. <span data-ttu-id="e6d3e-569">Vælg **Tilføj eksisterende**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-569">Select **Add existing**.</span></span>
3. <span data-ttu-id="e6d3e-570">Vælg **Kør forespørgsel** på siden **Føj testsager til pakke**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-570">On the **Add test cases to suite** page, select **Run query**.</span></span>
4. <span data-ttu-id="e6d3e-571">Vælg den nye testsag, der blev oprettet til **Frigiv et produkt**, og vælg derefter **Tilføj testsager** i nederste højre hjørne af siden (denne knap vises ikke i følgende illustration).</span><span class="sxs-lookup"><span data-stu-id="e6d3e-571">Select the new test case that was created for **Release a product**, and then select **Add test cases** in the lower-right corner of the page (this button isn't shown in the following illustration).</span></span>

    ![Siden Føj testsager til pakke](./media/setup_rsa_tool_100.png)

    <span data-ttu-id="e6d3e-573">Testpakken har nu to testsager.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-573">The test suite now has two test cases.</span></span>

    ![To testsager i testpakken](./media/setup_rsa_tool_101.png)

#### <a name="load-test-cases-into-rsat"></a><span data-ttu-id="e6d3e-575">Indlæse testsager i RSAT</span><span class="sxs-lookup"><span data-stu-id="e6d3e-575">Load test cases into RSAT</span></span>

1. <span data-ttu-id="e6d3e-576">Åbn RSAT, og vælg **Indlæs**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-576">Open RSAT, and select **Load**.</span></span>
2. <span data-ttu-id="e6d3e-577">Testsagerne indlæses, og du får vist en advarsel om, at "Handlingen vil overskrive Excel-testdatafiler, og lokale ændringer vil gå tabt.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-577">The test cases are loaded, and you receive a warning that states, "This action will overwrite Excel test data files, local changes will be lost.</span></span> <span data-ttu-id="e6d3e-578">Vil du fortsætte?"</span><span class="sxs-lookup"><span data-stu-id="e6d3e-578">Do you want to proceed?"</span></span> <span data-ttu-id="e6d3e-579">Vælg **Ja** for at opdatere Excel-parameterfilerne i det lokale system, men ikke de Excel-parameterfiler, der er overført til Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-579">Select **Yes** to update the Excel parameter files in the local system but not the Excel parameter files that were uploaded to Azure DevOps.</span></span>

    ![Advarselsmeddelelse](./media/setup_rsa_tool_102.png)

    <span data-ttu-id="e6d3e-581">Begge testsager indlæses sammen med Excel-parameterfilen til den første testsag.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-581">Both the test cases are loaded, together with the Excel parameter file for the first test case.</span></span> <span data-ttu-id="e6d3e-582">Da du valgte **Overførsel** i sidste kørsel, hentes parameterfilerne fra Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-582">Because you selected **Upload** in the last run, the parameter files are pulled from Azure DevOps.</span></span>

    ![Testsager er indlæst](./media/setup_rsa_tool_103.png)

3. <span data-ttu-id="e6d3e-584">Vælg kun den anden testsag, og vælg derefter **Ny \> Generér testkørsels- og parameterfiler**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-584">Select only the second test case, and then select **New \> Generate test execution and parameter files**.</span></span>

#### <a name="edit-the-excel-parameter-file"></a><span data-ttu-id="e6d3e-585">Redigere Excel-parameterfilen</span><span class="sxs-lookup"><span data-stu-id="e6d3e-585">Edit the Excel parameter file</span></span>

1. <span data-ttu-id="e6d3e-586">Vælg kun den anden testsag, og vælg derefter **Rediger** for at åbne den tilsvarende Excel-parameterfil.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-586">Select only the second test case, and then select **Edit** to open the corresponding Excel parameter file.</span></span>
2. <span data-ttu-id="e6d3e-587">Kopiér den gemte variabel **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** (se afsnittet [Redigere en eksisterende opgaveregistrering for at oprette en gemt variabel](#modify-an-existing-task-recording-to-create-a-saved-variable)) fra den første testsag til alle de felter, hvor produktnummeret bruges.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-587">Copy the **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** saved variable (see the [Modify an existing task recording to create a saved variable](#modify-an-existing-task-recording-to-create-a-saved-variable) section) from the first test case into all the fields where the product number is used.</span></span> <span data-ttu-id="e6d3e-588">I dette tilfælde skal du kopiere variablen til felterne **Produktnummer** og **Valider produktnummer** på arket **EcoResProductListPage**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-588">In this case, you copy the variable into the **Product number** and **Validate Product number** fields on the **EcoResProductListPage** sheet.</span></span>

    ![Felterne Produktnummer og Valider produktnummer](./media/setup_rsa_tool_104.png)

    > [!NOTE]
    > <span data-ttu-id="e6d3e-590">Der kan kun overføres variabler mellem testene under den samme testkørsel.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-590">Variables can be passed between tests only during the same test run.</span></span> <span data-ttu-id="e6d3e-591">Navnene på variablerne skal være helt ens.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-591">The names of the variables must match exactly.</span></span>

3. <span data-ttu-id="e6d3e-592">Vælg **Gem**, og luk derefter Excel-projektmappen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-592">Select **Save**, and then close the Excel workbook.</span></span>
4. <span data-ttu-id="e6d3e-593">Vælg **Overfør** for at gemme de ændringer, du har foretaget, i Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-593">Select **Upload** to save the changes that you made to the Excel parameter file.</span></span>

#### <a name="run-the-chained-test-cases"></a><span data-ttu-id="e6d3e-594">Køre de sammenkædede testsager</span><span class="sxs-lookup"><span data-stu-id="e6d3e-594">Run the chained test cases</span></span>

1. <span data-ttu-id="e6d3e-595">Vælg begge testsager, og vælg derefter **Kør**.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-595">Select both the test cases, and then select **Run**.</span></span>
2. <span data-ttu-id="e6d3e-596">Kontrollér, at begge testsager er bestået.</span><span class="sxs-lookup"><span data-stu-id="e6d3e-596">Verify that both test cases have passed.</span></span>
