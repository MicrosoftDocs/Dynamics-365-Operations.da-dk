...
>
> <span data-ttu-id="3a3af-141">Hvis reservationshierarkiet indeholder serienummerdimensionen (der altid skal være under niveauet **Batchnummer**), og hvis du har aktiveret batchspecifik reservation for batchnummeret, fortsætter systemet med at håndtere serienummerreservation og plukoperationer baseret på de regler, der gælder for reservationspolitikken "Serie-under\[lokation\]".</span><span class="sxs-lookup"><span data-stu-id="3a3af-141">If your reservation hierarchy includes the serial number dimension (which must always be below the **Batch number** level), and if you've turned on batch-specific reservation for the batch number, the system will continue to handle serial number reservation and picking operations, based on the rules that apply to the "Serial-below\[location\]" reservation policy.</span></span>
...
## <a name="example-scenario-batch-number-allocation"></a><span data-ttu-id="3a3af-164">Eksempel på scenario: batchnummertildeling</span><span class="sxs-lookup"><span data-stu-id="3a3af-164">Example scenario: Batch number allocation</span></span>
...
1. <span data-ttu-id="3a3af-177">Angiv produktets tre masterdataparametre ved hjælp af følgende værdier:</span><span class="sxs-lookup"><span data-stu-id="3a3af-177">Set the product's three master data parameters by using these values:</span></span>
    ...
    - <span data-ttu-id="3a3af-178">Vælg **Artikler** i feltet **Lagringsdimensionsgruppe**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-178">In the **Storage dimension group** field, select **Ware**.</span></span>
...
5. <span data-ttu-id="3a3af-209">Vælg **Lager** i oversigtspanelet **Salgsordrelinjer**, og vælg derefter **Batchreservation** i gruppen **Vedligehold**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-209">On the **Sales order lines** FastTab, select **Inventory**, and then, in the **Maintain** group, select **Batch reservation**.</span></span> <span data-ttu-id="3a3af-210">På siden **Batchreservation** vises en liste over batches, der er tilgængelige til reservation for ordrelinjen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-210">The **Batch reservation** page shows a list of batches that are available for reservation for the order line.</span></span> <span data-ttu-id="3a3af-211">I dette eksempel vises antallet **20** for batchnummer **B11** og antallet **10** for batchnummer **B22**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-211">For this example, it shows a quantity of **20** for batch number **B11** and a quantity of **10** for batch number **B22**.</span></span> <span data-ttu-id="3a3af-212">Bemærk, at der ikke kan opnås adgang til siden **Batchreservation** fra en linje, hvis varen på linjen er tilknyttet reservationshierarkiet "Batch-under\[lokation\]", medmindre den er konfigureret til at tillade batchspecifik reservation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-212">Note that the **Batch reservation** page cannot be accessed from a line if the item on that line is associated with "Batch-below\[location\]" reservation hierarchy unless it is set up to allow batch-specific reservation.</span></span>
...
6. <span data-ttu-id="3a3af-231">På siden **Batchreservation** skal du vælge linjen for batch **B11** og derefter vælge **Reserver linje**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-231">On the **Batch reservation** page, select the line for batch **B11**, and then select **Reserve line**.</span></span> <span data-ttu-id="3a3af-232">Der er ingen foruddefineret logik ved tildeling af lokationer og id'er under automatisk reservation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-232">There is no designated logic for assigning locations and license plates during automatic reservation.</span></span> <span data-ttu-id="3a3af-233">Du kan angive antallet manuelt i feltet **Reservation**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-233">You can manually enter the quantity in the **Reservation** field.</span></span> <span data-ttu-id="3a3af-234">Bemærk, at oversigtspanelet **Batchnumre bundet til kildelinje**, batch **B11** er vist som **Bindende**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-234">Notice that, on the **Batch numbers committed to source line** FastTab, batch **B11** is shown as **Committed**.</span></span>
...
## <a name="flexible-license-plate-reservation"></a><span data-ttu-id="3a3af-273">Fleksibel reservation af id</span><span class="sxs-lookup"><span data-stu-id="3a3af-273">Flexible license plate reservation</span></span>
...
1. <span data-ttu-id="3a3af-286">**Funktionsnavn:** *Fleksibel reservation for dimension på lagerstedsniveau*</span><span class="sxs-lookup"><span data-stu-id="3a3af-286">**Feature name:** *Flexible warehouse-level dimension reservation*</span></span>
...
- <span data-ttu-id="3a3af-298">Reference (kun *Salgsordre*-værdien understøttes).</span><span class="sxs-lookup"><span data-stu-id="3a3af-298">Reference (Only the *Sales order* value is supported.)</span></span>
...
<span data-ttu-id="3a3af-311">Dette scenario viser, hvordan du kan oprette og behandle en ordrebekræftet id-reservation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-311">This scenario shows how to set up and process an order-committed license plate reservation.</span></span>
...
### <a name="create-two-released-products"></a><span data-ttu-id="3a3af-327">Oprette to frigivne produkter</span><span class="sxs-lookup"><span data-stu-id="3a3af-327">Create two released products</span></span>
...
1. <span data-ttu-id="3a3af-381">Tilføj en anden salgsordrelinje, der har følgende indstillinger:</span><span class="sxs-lookup"><span data-stu-id="3a3af-381">Add a second sales order line that has the following settings:</span></span>
    ...
    - <span data-ttu-id="3a3af-382">**Varenummer:** *Vare2*</span><span class="sxs-lookup"><span data-stu-id="3a3af-382">**Item number:** *Item2*</span></span>
...
1. <span data-ttu-id="3a3af-394">Hvis du vil reservere en vare på et bestemt id, skal du vælge **Ny** i Excel-tilføjelsesprogrammet for at tilføje en reservationslinje og derefter angive følgende værdier:</span><span class="sxs-lookup"><span data-stu-id="3a3af-394">To reserve an item on a specific license plate, in the Excel add-in, select **New** to add a reservation line, and then set the following values:</span></span>
    ...
    - <span data-ttu-id="3a3af-395">**Parti-id:** Angiv den **Parti-id**-værdi, du har fundet for salgsordrelinjen til *Vare1*.</span><span class="sxs-lookup"><span data-stu-id="3a3af-395">**Lot ID:** Enter the **Lot ID** value that you found for the sales order line for *Item1*.</span></span>
...
1. <span data-ttu-id="3a3af-413">Gå til oversigtspanelet **Salgsordrelinjer**, og vælg **Arbejdsdetaljer** i menuen **Lagersted**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-413">On the **Sales order lines** FastTab, on the **Warehouse** menu, select **Work details**.</span></span>

    <span data-ttu-id="3a3af-414">Når der foretages reservation for et bestemt batch, bruger systemet ikke lokationsdirektiver, når det opretter arbejdet for salgsordren, der bruger id-reservation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-414">As when reservation is done for a specific batch, the system doesn't use location directives when it creates the work for the sales order that uses license plate reservation.</span></span> <span data-ttu-id="3a3af-415">Da den ordrebekræftede reservation angiver alle lagerdimensioner, herunder lokationen, skal du ikke bruge lokationsdirektiver, da disse lagerdimensioner kun er angivet i arbejdet.</span><span class="sxs-lookup"><span data-stu-id="3a3af-415">Because the order-committed reservation specifies all the inventory dimensions, including the location, location directives don't have to be used, because those inventory dimensions are just entered in the work.</span></span> <span data-ttu-id="3a3af-416">De vises i afsnittet **Fra lagerdimensioner** på siden **Arbejdslagertransaktioner**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-416">They are shown in the **From inventory dimensions** section on the **Work inventory transactions** page.</span></span>

    > [!NOTE]
    > <span data-ttu-id="3a3af-417">Når arbejdet er oprettet, fjernes varens lagertransaktion, hvor feltet **Reference** er angivet til *Ordrebekræftet reservation*.</span><span class="sxs-lookup"><span data-stu-id="3a3af-417">After the work is created, the item's inventory transaction where the **Reference** field is set to *Order-committed reservation* is removed.</span></span> <span data-ttu-id="3a3af-418">Lagertransaktionen, hvor feltet **Reference** er angivet til *Arbejde*, indeholder nu den fysiske reservation af lagerdimensionerne for alle antal.</span><span class="sxs-lookup"><span data-stu-id="3a3af-418">The inventory transaction where the **Reference** field is set to *Work* now holds the physical reservation for all the quantity's inventory dimensions.</span></span>

1. <span data-ttu-id="3a3af-419">På mobilenheden skal du afslutte plukning og placere arbejdet ved hjælp af et menupunkt, hvor afkrydsningsfeltet **Håndter efter id** er markeret.</span><span class="sxs-lookup"><span data-stu-id="3a3af-419">On the mobile device, finish picking and putting the work by using a menu item where the **Handle by license plate** check box is selected.</span></span>

    > [!NOTE]
    > <span data-ttu-id="3a3af-420">Med funktionen **Håndter efter id** kan du behandle hele id'et.</span><span class="sxs-lookup"><span data-stu-id="3a3af-420">The **Handle by license plate** functionality helps you process the whole license plate.</span></span> <span data-ttu-id="3a3af-421">Hvis du skal behandle en del af id'et, kan du ikke bruge denne funktion.</span><span class="sxs-lookup"><span data-stu-id="3a3af-421">If you must process part of the license plate, you can't use this functionality.</span></span>
    >
    > <span data-ttu-id="3a3af-422">Det anbefales, at du har genereret et separat arbejde for hvert enkelt id.</span><span class="sxs-lookup"><span data-stu-id="3a3af-422">We recommend that you have separate work generated for each license plate.</span></span> <span data-ttu-id="3a3af-423">Du kan opnå dette resultat ved at bruge funktionen **Arbejdshovedpauser** på siden **Arbejdsskabelon**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-423">To achieve this result, use the **Work header breaks** feature on the **Work template** page.</span></span>

    <span data-ttu-id="3a3af-424">Id *LP02* plukkes nu til salgsordrelinjer og placeres på lokationen *Lagerport*.</span><span class="sxs-lookup"><span data-stu-id="3a3af-424">License plate *LP02* is now picked for sales order lines and put to the *Baydoor* location.</span></span> <span data-ttu-id="3a3af-425">På dette tidspunkt er det klar til at blive læsset og sendt til kundens adresse.</span><span class="sxs-lookup"><span data-stu-id="3a3af-425">At this point, it's ready to be loaded and dispatched to the customer.</span></span>

## <a name="exception-handling-of-warehouse-work-that-has-order-committed-batch-numbers"></a><span data-ttu-id="3a3af-426">Undtagelseshåndtering af lagerstedsarbejde, der har ordrebekræftede batchnumre</span><span class="sxs-lookup"><span data-stu-id="3a3af-426">Exception handling of warehouse work that has order-committed batch numbers</span></span>

<span data-ttu-id="3a3af-427">Lagerstedsarbejde for plukning af ordrebekræftede batchnumre er underlagt samme standardhåndtering af undtagelser for lagersted og handlinger som almindeligt arbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-427">Warehouse work for picking order-committed batch numbers is subject to the same standard warehouse exception handling and actions as regular work.</span></span> <span data-ttu-id="3a3af-428">Generelt kan det åbne arbejde eller den åbne arbejdslinje annulleres, den kan afbrydes, fordi en brugerlokation er fuld, den kan være kort plukket, og den kan opdateres på grund af en bevægelse.</span><span class="sxs-lookup"><span data-stu-id="3a3af-428">In general, the open work or work line can be canceled, it can be interrupted because a user location is full, it can be short-picked, and it can be updated because of a movement.</span></span> <span data-ttu-id="3a3af-429">På samme måde kan den plukkede mængde arbejde, der allerede er udført, reduceres, eller arbejdet kan tilbageføres.</span><span class="sxs-lookup"><span data-stu-id="3a3af-429">Likewise, the picked quantity of work that has already been completed can be reduced, or the work can be reversed.</span></span>

<span data-ttu-id="3a3af-430">Følgende nøgleregel anvendes til alle disse handlinger af typen undtagelseshåndtering: Det batchnummer, der er reserveret til kunden, kan aldrig erstattes med et andet batchnummer, men lagerdimensionerne (lokation og id) kan ændres via enten en manuel opdatering fra brugerens side eller en automatisk opdatering udført af systemet.</span><span class="sxs-lookup"><span data-stu-id="3a3af-430">The following key rule is applied to all these exception handling actions: the batch number that was reserved for the customer can never be replaced with a different batch number, but its storage dimensions (location and license plate) can be changed through either a manual update by the user or an automatic update by the system.</span></span> <span data-ttu-id="3a3af-431">Den automatiske opdatering er baseret på den samme tilfældige tildeling af lagringsdimensioner, der anvendes, når en bestemt batch blev reserveret automatisk, men der ikke blev angivet nogen lagerdimensioner.</span><span class="sxs-lookup"><span data-stu-id="3a3af-431">The automatic update is based on the same random assignment of storage dimensions that applied when a specific batch was automatically reserved but no storage dimensions were specified.</span></span>

### <a name="example-scenario"></a><span data-ttu-id="3a3af-432">Eksempelscenario</span><span class="sxs-lookup"><span data-stu-id="3a3af-432">Example scenario</span></span>

<span data-ttu-id="3a3af-433">Et eksempel på dette scenario er en situation, hvor tidligere udført arbejde ikke kan plukkes ved hjælp af funktionen **Reducer det antal, der er plukket**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-433">An example of this scenario is a situation where previously completed work is being unpicked by using the **Reduce picked quantity** function.</span></span> <span data-ttu-id="3a3af-434">I dette eksempel antages det, at du allerede har udført de trin, der er beskrevet i [Eksempel på scenario: batchnummertildeling](#Example-batch-allocation).</span><span class="sxs-lookup"><span data-stu-id="3a3af-434">This example assumes that you've already completed the steps that are described in [Example scenario: Batch number allocation](#Example-batch-allocation).</span></span> <span data-ttu-id="3a3af-435">Det er en fortsættelse af dette eksempel.</span><span class="sxs-lookup"><span data-stu-id="3a3af-435">It continues from that example.</span></span>

1. <span data-ttu-id="3a3af-436">Gå til **Lokationsstyring** \> **Laster** \> **Aktive laster**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-436">Go to **Warehouse management** \> **Loads** \> **Active loads**.</span></span>
2. <span data-ttu-id="3a3af-437">Vælg den last, der blev oprettet i forbindelse med forsendelsen af salgsordren.</span><span class="sxs-lookup"><span data-stu-id="3a3af-437">Select the load that was created in connection with the shipment of your sales order.</span></span>
3. <span data-ttu-id="3a3af-438">Vælg **Reducer det antal, der er plukket** i oversigtspanelet **Indlæs ordrelinjer**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-438">From the **Load order lines** FastTab, select **Reduce picked quantity**.</span></span>
4. <span data-ttu-id="3a3af-439">Vælg **FL-001** i feltet **Flyt til lokation** på siden **Reducer det antal, der er plukket**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-439">On the **Reduce picked quantity** page, in the **Move to location** field, select **FL-001**.</span></span>
5. <span data-ttu-id="3a3af-440">Vælg **LP33** i feltet **Flyt til nummerplade**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-440">In the **Move to license plate** field, select **LP33**.</span></span>
6. <span data-ttu-id="3a3af-441">Angiv **10** i feltet **Lagerantal, hvor plukning skal fortrydes**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-441">In the grid, in the **Inventory quantity to unpick** field, enter **10**.</span></span>
7. <span data-ttu-id="3a3af-442">Vælg **OK**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-442">Select **OK**.</span></span>

<span data-ttu-id="3a3af-443">Følgende er resultatet af handlingen til fortrydelse af plukning:</span><span class="sxs-lookup"><span data-stu-id="3a3af-443">Here are the results of the unpicking action:</span></span>

- <span data-ttu-id="3a3af-444">Status for det tidligere lukkede arbejde angives til **Annulleret**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-444">The status of the previously closed work is set to **Canceled**.</span></span>
- <span data-ttu-id="3a3af-445">Der oprettes nyt arbejde af typen **Lagerbevægelser** for det ikke-plukkede antal på **10** for batchnummer **B11**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-445">New work of the **Inventory movement** type is created for the unpicked quantity of **10** for batch number **B11**.</span></span> <span data-ttu-id="3a3af-446">Dette arbejde repræsenterer bevægelsen fra lokationen **Baydoor** til id **LP33** på lokation **FL-001**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-446">This work represents the movement from the **Baydoor** location to license plate **LP33** in location **FL-001**.</span></span> <span data-ttu-id="3a3af-447">Status er angivet til **Lukket**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-447">The status is set to **Closed**.</span></span>
- <span data-ttu-id="3a3af-448">Systemet reserverer igen det batchnummer, der oprindeligt blev bestilt, og tildeler lokation og id'er.</span><span class="sxs-lookup"><span data-stu-id="3a3af-448">The system re-reserves the batch number that was originally ordered, and assigns the location and license plate IDs.</span></span> <span data-ttu-id="3a3af-449">(Denne proces svarer til at køre funktionen **Reserver linje** for ordrelinjen for et bestemt batchnummer).</span><span class="sxs-lookup"><span data-stu-id="3a3af-449">(This process is equivalent to running the **Reserve line** function for the order line for a given batch number).</span></span> <span data-ttu-id="3a3af-450">Derfor vises **B11** som bekræftet i oversigtspanelet **Batchnumre bundet til kildelinje** på siden **Batchreservation**, og feltet **Reservation** indeholder antallet **10** for batchnummer **B11**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-450">As a result, batch **B11** is shown as committed on the **Batch numbers committed to source line** FastTab of the **Batch reservation** page, and the **Reservation** field contains a quantity of **10** for batch number **B11**.</span></span> <span data-ttu-id="3a3af-451">Desuden angives feltet **Lokation** til **FL-001**, og feltet **Nummerplade** angives til **LP11**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-451">Additionally, the **Location** field is set to **FL-001**, and the **License plate** field is set to **LP11**.</span></span> <span data-ttu-id="3a3af-452">(Du kan føje disse felter til gitteret, hvis de ikke er synlige).</span><span class="sxs-lookup"><span data-stu-id="3a3af-452">(You can add these fields to the grid if they aren't visible.)</span></span>

<span data-ttu-id="3a3af-453">Følgende tabeller indeholder en oversigt, der viser, hvordan systemet håndterer ordrebekræftet batchreservation for specifikke lagerstedshandlinger.</span><span class="sxs-lookup"><span data-stu-id="3a3af-453">The following tables provide an overview that shows how the system handles order-committed batch reservation for specific warehouse actions.</span></span> <span data-ttu-id="3a3af-454">Hvis du vil fortolke indholdet i tabellerne, skal du antage, at hver lagerstedshandling køres i konteksten af det eksisterende lagerarbejde, der stammer fra en ordrebekræftet batchreservation, eller at udførelsen af de enkelte lagerhandlinger påvirker arbejde af den pågældende type.</span><span class="sxs-lookup"><span data-stu-id="3a3af-454">To interpret the content in the tables, assume that each warehouse action is run in the context of existing warehouse work that originates from an order-committed batch reservation, or that execution of each warehouse action affects work of that type.</span></span>

> [!NOTE]
> <span data-ttu-id="3a3af-455">I disse tabeller angiver kolonnen "Batchantal er tilgængeligt", om der findes et tilgængeligt batchantal ud over det antal, der enten allerede er reserveret for de aktuelle ordrebekræftede reservationer, eller som allerede er reserveret af det lagerstedsarbejde, der stammer fra reservationer af den pågældende type.</span><span class="sxs-lookup"><span data-stu-id="3a3af-455">In these tables, the "Batch quantity is available" column indicates whether a batch quantity is available in addition to the quantity that is either already reserved for the current order-committed reservations or already reserved by the warehouse work that originates from reservations of that type.</span></span>

#### <a name="override-the-pick-location-on-the-open-work"></a><span data-ttu-id="3a3af-456">Tilsidesæt pluklokationen for det åbne arbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-456">Override the pick location on the open work</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="3a3af-457">Vigtig opsætningsparameter</span><span class="sxs-lookup"><span data-stu-id="3a3af-457">Key setup parameter</span></span></th>
<th><span data-ttu-id="3a3af-458">Batchantal er tilgængeligt</span><span class="sxs-lookup"><span data-stu-id="3a3af-458">Batch quantity is available</span></span></th>
<th><span data-ttu-id="3a3af-459">Vigtige brugertrin</span><span class="sxs-lookup"><span data-stu-id="3a3af-459">Key user steps</span></span></th>
<th><span data-ttu-id="3a3af-460">Lagerstedsarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-460">Warehouse work</span></span></th>
<th><span data-ttu-id="3a3af-461">Ordrebekræftet batchreservation</span><span class="sxs-lookup"><span data-stu-id="3a3af-461">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="3a3af-462">Indstillingen <strong>Tillad tilsidesættelse af pluklokation</strong> er aktiveret for arbejderen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-462">The <strong>Allow pick location override</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="3a3af-463">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-463">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-464">Vælg menupunktet <strong>Tilsidesæt lokation</strong> på lagerstedsappen, når du starter pluk af arbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-464">Select the <strong>Override location</strong> menu item on the warehouse app when you start picking work.</span></span></li>
<li><span data-ttu-id="3a3af-465">Vælg <strong>Foreslå</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-465">Select <strong>Suggest</strong>.</span></span></li>
<li><span data-ttu-id="3a3af-466">Bekræft den nye lokation, der foreslås, på basis af tilgængeligheden af batchantal.</span><span class="sxs-lookup"><span data-stu-id="3a3af-466">Confirm the new location that is suggested based on batch quantity availability.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-467">Følgende handlinger finder sted i det aktuelle arbejde:</span><span class="sxs-lookup"><span data-stu-id="3a3af-467">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="3a3af-468">Lokationen på pluklinjen opdateres til den nye lokation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-468">The location on the pick line is updated to the new location.</span></span> <span data-ttu-id="3a3af-469">(Hvis lokaliteten er id-styret, tildeles et tilfældigt id til lagerbeholdningstransaktionen, og arbejderen kan plukke fra et hvilken som helst id, der har tilgængeligt antal).</span><span class="sxs-lookup"><span data-stu-id="3a3af-469">(If the location is license plate–controlled, a random license plate is assigned to the work inventory transaction, and the worker can pick from any license plate that has available quantity.)</span></span></li>
<li><span data-ttu-id="3a3af-470">Hvis antallet findes på mere end ét id på den nye lokation, opdeles den oprindelige pluklinje i flere linjer, så den svarer til hvert enkelt id.</span><span class="sxs-lookup"><span data-stu-id="3a3af-470">If the quantity is found on more than one license plate in the new location, the original pick line is split into multiple lines to match each license plate.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-471">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-471">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-472">Ingen</span><span class="sxs-lookup"><span data-stu-id="3a3af-472">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-473">Vælg menupunktet <strong>Tilsidesæt lokation</strong> på lagerstedsappen, når du starter pluk af arbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-473">Select the <strong>Override location</strong> menu item on the warehouse app when you start picking work.</span></span></li>
<li><span data-ttu-id="3a3af-474">Angiv en lokation manuelt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-474">Manually enter a location.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-475">Handlingen <strong>Tilsidesæt lokation</strong> er ikke mulig.</span><span class="sxs-lookup"><span data-stu-id="3a3af-475">The <strong>Override location</strong> action isn't possible.</span></span> <span data-ttu-id="3a3af-476">Det mislykkes, og der udløses en fejl.</span><span class="sxs-lookup"><span data-stu-id="3a3af-476">It fails, and an error is thrown.</span></span></td>
<td><span data-ttu-id="3a3af-477">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-477">Not applicable</span></span></td>
</tr>
</tbody>
</table>

#### <a name="full-button--split-a-work-line-because-of-overflow-on-the-user-location"></a><span data-ttu-id="3a3af-478">Knappen Fuld – opdel en arbejdslinje på grund af overløb på brugerens lokation</span><span class="sxs-lookup"><span data-stu-id="3a3af-478">Full button – Split a work line because of overflow on the user location</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="3a3af-479">Vigtig opsætningsparameter</span><span class="sxs-lookup"><span data-stu-id="3a3af-479">Key setup parameter</span></span></th>
<th><span data-ttu-id="3a3af-480">Batchantal er tilgængeligt</span><span class="sxs-lookup"><span data-stu-id="3a3af-480">Batch quantity is available</span></span></th>
<th><span data-ttu-id="3a3af-481">Vigtige brugertrin</span><span class="sxs-lookup"><span data-stu-id="3a3af-481">Key user steps</span></span></th>
<th><span data-ttu-id="3a3af-482">Lagerstedsarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-482">Warehouse work</span></span></th>
<th><span data-ttu-id="3a3af-483">Ordrebekræftet batchreservation</span><span class="sxs-lookup"><span data-stu-id="3a3af-483">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td><span data-ttu-id="3a3af-484">Indstillingen <strong>Tillad opdeling af arbejde</strong> er aktiveret for menupunktet mobilenhed.</span><span class="sxs-lookup"><span data-stu-id="3a3af-484">The <strong>Allow splitting of work</strong> option is enabled on the mobile device menu item.</span></span></td>
<td><span data-ttu-id="3a3af-485">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-485">Not applicable</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-486">Vælg menupunktet <strong>Fuld</strong> på lagerstedsapp, når du behandler plukarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-486">Select the <strong>Full</strong> menu item on the warehouse app when you process picking work.</span></span></li>
<li><span data-ttu-id="3a3af-487">I feltet <strong>Plukantal</strong> skal du angive en del af antallet af det krævede pluk for at angive den fulde kapacitet.</span><span class="sxs-lookup"><span data-stu-id="3a3af-487">In the <strong>Pick Qty</strong> field, enter a partial quantity of the required pick to indicate the full capacity.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="3a3af-488">I det aktuelle arbejde opdateres antallet med det resterende antal, der skal plukkes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-488">On the current work, the quantity is updated to the remaining quantity that must be picked.</span></span></li>
<li><span data-ttu-id="3a3af-489">Nyt arbejde for det plukkede antal oprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-489">New work for the picked quantity is created and closed.</span></span></li>
</ul></td>
<td><span data-ttu-id="3a3af-490">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-490">Not applicable</span></span></td>
</tr>
</tbody>
</table>

#### <a name="reduce-the-picked-quantity-of-completed-work-from-a-load"></a><span data-ttu-id="3a3af-491">Reducer det plukkede antal af fuldført arbejde (fra en last)</span><span class="sxs-lookup"><span data-stu-id="3a3af-491">Reduce the picked quantity of completed work (from a load)</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="3a3af-492">Vigtig opsætningsparameter</span><span class="sxs-lookup"><span data-stu-id="3a3af-492">Key setup parameter</span></span></th>
<th><span data-ttu-id="3a3af-493">Batchantal er tilgængeligt</span><span class="sxs-lookup"><span data-stu-id="3a3af-493">Batch quantity is available</span></span></th>
<th><span data-ttu-id="3a3af-494">Vigtige brugertrin</span><span class="sxs-lookup"><span data-stu-id="3a3af-494">Key user steps</span></span></th>
<th><span data-ttu-id="3a3af-495">Lagerstedsarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-495">Warehouse work</span></span></th>
<th><span data-ttu-id="3a3af-496">Ordrebekræftet batchreservation</span><span class="sxs-lookup"><span data-stu-id="3a3af-496">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="3a3af-497">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-497">Not applicable</span></span></td>
<td><span data-ttu-id="3a3af-498">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-498">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-499">Åbn siden <strong>Reducer det antal, der er plukket</strong> antal fra lastlinjen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-499">Open the <strong>Reduce picked quantity</strong> page from the load line.</span></span></li>
<li><span data-ttu-id="3a3af-500">Angiv hele antallet, plukning skal fortrydes for.</span><span class="sxs-lookup"><span data-stu-id="3a3af-500">Enter the full quantity to unpick.</span></span></li>
<li><span data-ttu-id="3a3af-501">Vælg en "flyt til"-lokation/id.</span><span class="sxs-lookup"><span data-stu-id="3a3af-501">Select a "move to" location/license plate.</span></span></li>
</ol>
</td>
<td>
<ul> 
<li><span data-ttu-id="3a3af-502">Arbejde, der er tilknyttet lastlinjen, annulleres.</span><span class="sxs-lookup"><span data-stu-id="3a3af-502">Work that is associated with the load line is canceled.</span></span></li>
<li><span data-ttu-id="3a3af-503">Nyt arbejde for lagerbevægelsen oprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-503">New work for the inventory movement is created and closed.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-504">Antallet reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-504">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-505">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-505">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-506">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-506">No</span></span></td>
<td><span data-ttu-id="3a3af-507">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-507">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-508">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-508">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-509">Antallet reserveres igen for samme batch og for samme lokation og id (hvis lokationen er id-styret), der blev angivet under annullering af pluk.</span><span class="sxs-lookup"><span data-stu-id="3a3af-509">The quantity is re-reserved for the same batch, and for the same location and license plate (if the location is license plate–controlled) that were entered during unpicking.</span></span></td>
</tr>
</tbody>
</table>

#### <a name="move-an-item-within-a-warehouse"></a><span data-ttu-id="3a3af-510">Flyt en vare inden for et lagersted</span><span class="sxs-lookup"><span data-stu-id="3a3af-510">Move an item within a warehouse</span></span>

> [!NOTE]
> <span data-ttu-id="3a3af-511">Denne lagerstedshandling gælder kun for flytning af typen **Arbejdsoprettelsesproces**, ikke for bevægelse efter skabelon.</span><span class="sxs-lookup"><span data-stu-id="3a3af-511">This warehouse action is applicable only to movement of the **Work creation process** type, not to movement by template.</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="3a3af-512">Vigtig opsætningsparameter</span><span class="sxs-lookup"><span data-stu-id="3a3af-512">Key setup parameter</span></span></th>
<th><span data-ttu-id="3a3af-513">Batchantal er tilgængeligt</span><span class="sxs-lookup"><span data-stu-id="3a3af-513">Batch quantity is available</span></span></th>
<th><span data-ttu-id="3a3af-514">Vigtige brugertrin</span><span class="sxs-lookup"><span data-stu-id="3a3af-514">Key user steps</span></span></th>
<th><span data-ttu-id="3a3af-515">Lagerstedsarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-515">Warehouse work</span></span></th>
<th><span data-ttu-id="3a3af-516">Ordrebekræftet batchreservation</span><span class="sxs-lookup"><span data-stu-id="3a3af-516">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="3a3af-517">Indstillingen <strong>Tillad flytning af lager med tilknyttet arbejde</strong> er aktiveret for arbejderen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-517">The <strong>Allow movement of inventory with associated work</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="3a3af-518">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-518">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-519">Start en bevægelse på lagerstedsappen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-519">Start a movement on the warehouse app.</span></span></li>
<li><span data-ttu-id="3a3af-520">Angiv "fra"- og "til"-lokationer.</span><span class="sxs-lookup"><span data-stu-id="3a3af-520">Enter "from" and "to" locations.</span></span></li>
</ol></td>
<td>
<ul>
<li><span data-ttu-id="3a3af-521">I alt eksisterende arbejde, der påvirkes af flytningen, opdateres plukpladsen til den nye "til"-lokation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-521">On all existing work that is affected by the move, the pick location is updated to the new "to" location.</span></span></li>
<li><span data-ttu-id="3a3af-522">Nyt arbejde for lagerbevægelsen oprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-522">New work for the inventory movement is created and closed.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-523">Alle eksisterende reservationer, der påvirkes af antalsbevægelsen fra den angivne lokation, reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-523">All existing reservations that are affected by the quantity movement from the given location are re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-524">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-524">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-525">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-525">No</span></span></td>
<td><span data-ttu-id="3a3af-526">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-526">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-527">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-527">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-528">Alle eksisterende reservationer, der påvirkes af antalsbevægelsen fra den angivne lokation, reserveres igen for den samme batch og for den nye "til"-lokation og det nye id (hvis lokationen er id-styret).</span><span class="sxs-lookup"><span data-stu-id="3a3af-528">All existing reservations that are affected by the quantity movement from the given location are re-reserved for the same batch, and for the new "to" location and license plate (if the location is license plate–controlled).</span></span></td>
</tr>
</tbody>
</table>

#### <a name="reverse-the-picked-quantity-of-completed-work-from-a-load-or-a-wave"></a><span data-ttu-id="3a3af-529">Tilbagefør det plukkede antal af fuldført arbejde (fra en belastning eller en bølge)</span><span class="sxs-lookup"><span data-stu-id="3a3af-529">Reverse the picked quantity of completed work (from a load or a wave)</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="3a3af-530">Vigtig opsætningsparameter</span><span class="sxs-lookup"><span data-stu-id="3a3af-530">Key setup parameter</span></span></th>
<th><span data-ttu-id="3a3af-531">Batchantal er tilgængeligt</span><span class="sxs-lookup"><span data-stu-id="3a3af-531">Batch quantity is available</span></span></th>
<th><span data-ttu-id="3a3af-532">Vigtige brugertrin</span><span class="sxs-lookup"><span data-stu-id="3a3af-532">Key user steps</span></span></th>
<th><span data-ttu-id="3a3af-533">Lagerstedsarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-533">Warehouse work</span></span></th>
<th><span data-ttu-id="3a3af-534">Ordrebekræftet batchreservation</span><span class="sxs-lookup"><span data-stu-id="3a3af-534">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='6'><span data-ttu-id="3a3af-535">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-535">Not applicable</span></span></td>
<td><span data-ttu-id="3a3af-536">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-536">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-537">Åbn siden <strong>Tilbagefør arbejde</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-537">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="3a3af-538">Vælg indstillingen <strong>Efterlad varer på aktuel lokation</strong> på siden med anmodning.</span><span class="sxs-lookup"><span data-stu-id="3a3af-538">On the request page, select the <strong>Leave items at current location</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-539">Alt arbejde, der er tilknyttet lasten, annulleres.</span><span class="sxs-lookup"><span data-stu-id="3a3af-539">All work that is associated with the load is canceled.</span></span></td>
<td><span data-ttu-id="3a3af-540">Antallet reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-540">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-541">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-541">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-542">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-542">No</span></span></td>
<td><span data-ttu-id="3a3af-543">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-543">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-544">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-544">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-545">Antallet reserveres igen for den samme batch og for den lokation og det id, hvor antallet blev placeret ved tilbageførsel.</span><span class="sxs-lookup"><span data-stu-id="3a3af-545">The quantity is re-reserved for the same batch, and for the location and license plate where the quantity was left upon reversal.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-546">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-546">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-547">Åbn siden <strong>Tilbagefør arbejde</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-547">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="3a3af-548">Vælg indstillingen <strong>Tildel varer til denne lokation</strong> på siden med anmodning.</span><span class="sxs-lookup"><span data-stu-id="3a3af-548">On the request page, select the <strong>Assign items to this location</strong> option.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="3a3af-549">Det aktuelle arbejde er annulleret.</span><span class="sxs-lookup"><span data-stu-id="3a3af-549">The current work is canceled.</span></span></li>
<li><span data-ttu-id="3a3af-550">Nyt arbejde for lagerbevægelsen oprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-550">New work for the inventory movement is created and closed.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-551">Antallet reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-551">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-552">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-552">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-553">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-553">No</span></span></td>
<td><span data-ttu-id="3a3af-554">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-554">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-555">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-555">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-556">Antallet reserveres igen for den samme batch og for den lokation og det id, som antallet blev tildelt ved tilbageførsel.</span><span class="sxs-lookup"><span data-stu-id="3a3af-556">The quantity is re-reserved for the same batch, and for the location and license plate that the quantity was assigned to upon reversal.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-557">Ja/nej</span><span class="sxs-lookup"><span data-stu-id="3a3af-557">Yes/No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-558">Åbn siden <strong>Tilbagefør arbejde</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-558">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="3a3af-559">Vælg indstillingen <strong>Flyt varer til denne lokation</strong> på siden med anmodning.</span><span class="sxs-lookup"><span data-stu-id="3a3af-559">On the request page, select the <strong>Move items to this location</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-560">Tilbageførsel understøttes ikke.</span><span class="sxs-lookup"><span data-stu-id="3a3af-560">Reversal isn't supported.</span></span></td>
<td><span data-ttu-id="3a3af-561">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-561">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-562">Ja/nej</span><span class="sxs-lookup"><span data-stu-id="3a3af-562">Yes/No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-563">Åbn siden <strong>Tilbagefør arbejde</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-563">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="3a3af-564">Vælg indstillingen <strong>Flyt varer på basis af lokationsvejledninger</strong> på siden med anmodning.</span><span class="sxs-lookup"><span data-stu-id="3a3af-564">On the request page, select the <strong>Move items based on location directives</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-565">Tilbageførsel understøttes ikke.</span><span class="sxs-lookup"><span data-stu-id="3a3af-565">Reversal isn't supported.</span></span> </td>
<td><span data-ttu-id="3a3af-566">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-566">Not applicable</span></span></td>
</tr>
</tbody>
</table>

#### <a name="short-pick-a-quantity--register-the-quantity-as-physically-not-found-at-the-locationlicense-plate-while-you-perform-picking-work"></a><span data-ttu-id="3a3af-567">Kort pluk et antal – Registrer antallet som fysisk ikke fundet ved lokationen/id'et, når du udfører plukarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-567">Short-pick a quantity – Register the quantity as physically not found at the location/license plate while you perform picking work</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="3a3af-568">Vigtig opsætningsparameter</span><span class="sxs-lookup"><span data-stu-id="3a3af-568">Key setup parameter</span></span></th>
<th><span data-ttu-id="3a3af-569">Batchantal er tilgængeligt</span><span class="sxs-lookup"><span data-stu-id="3a3af-569">Batch quantity is available</span></span></th>
<th><span data-ttu-id="3a3af-570">Vigtige brugertrin</span><span class="sxs-lookup"><span data-stu-id="3a3af-570">Key user steps</span></span></th>
<th><span data-ttu-id="3a3af-571">Lagerstedsarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-571">Warehouse work</span></span></th>
<th><span data-ttu-id="3a3af-572">Ordrebekræftet batchreservation</span><span class="sxs-lookup"><span data-stu-id="3a3af-572">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="3a3af-573">En arbejdsundtagelse af typen <strong>Kort pluk</strong> er angivet, hvor <strong>Vareomfordeling</strong> = <strong>Ingen</strong>, <strong>Reguler lager</strong> = <strong>Ja</strong>, og <strong>Fjern reservationer</strong> = <strong>Nej</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-573">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>None</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No</strong>.</span></span></td>
<td><span data-ttu-id="3a3af-574">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-574">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-575">Vælg menupunktet <strong>Kort pluk</strong> på lagerstedsappen, når du kører plukarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-575">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="3a3af-576">Angiv <strong>0</strong> i feltet <strong>Pluk antal</strong> (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-576">In the <strong>Pick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="3a3af-577">I feltet <strong>Årsag</strong> skal du angive <strong>Ingen omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-577">In the <strong>Reason</strong> field, enter <strong>No reallocation</strong>.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="3a3af-578">Det aktuelle arbejde er lukket, og det plukkede antal er 0 (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-578">The current work is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="3a3af-579">Der oprettes en lagertransaktion af typen <strong>Optælling</strong>, og afgangstypen <strong>Solgt</strong> oprettes for at repræsentere reguleringen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-579">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-580">Antallet reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-580">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-581">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-581">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-582">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-582">No</span></span></td>
<td><span data-ttu-id="3a3af-583">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-583">See the previous row.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="3a3af-584">Handlingen kort pluk mislykkes, og der udløses en fejl.</span><span class="sxs-lookup"><span data-stu-id="3a3af-584">The short-picking action fails, and an error is thrown.</span></span></li>
<li><span data-ttu-id="3a3af-585">Det aktuelle arbejde forbliver åbent.</span><span class="sxs-lookup"><span data-stu-id="3a3af-585">The current work remains open.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-586">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-586">Not applicable</span></span></td>
</tr>
<tr>
<td rowspan='2'><span data-ttu-id="3a3af-587">En arbejdsundtagelse af typen <strong>Kort pluk</strong> er angivet, hvor <strong>Vareomfordeling</strong> = <strong>Ingen</strong>, <strong>Reguler lager</strong> = <strong>Ja</strong>, og <strong>Fjern reservationer</strong> = <strong>Ja</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-587">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>None</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>Yes</strong>.</span></span></td>
<td><span data-ttu-id="3a3af-588">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-588">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-589">Vælg menupunktet <strong>Kort pluk</strong> på lagerstedsappen, når du kører plukarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-589">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="3a3af-590">Angiv <strong>0</strong> i feltet <strong>Pluk antal</strong> (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-590">In the <strong>Pick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="3a3af-591">I feltet <strong>Årsag</strong> skal du angive <strong>Ingen omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-591">In the <strong>Reason</strong> field, enter <strong>No reallocation</strong>.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="3a3af-592">Det aktuelle arbejde er lukket, og det plukkede antal er 0 (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-592">The current work is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="3a3af-593">Der oprettes en lagertransaktion af typen <strong>Optælling</strong>, og afgangstypen <strong>Solgt</strong> oprettes for at repræsentere reguleringen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-593">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-594">Alle eksisterende reservationer, der påvirkes af antalsreguleringen i den kort plukkede lokation, reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-594">All existing reservations that are affected by the quantity adjustment in the short-picked location are re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-595">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-595">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-596">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-596">No</span></span></td>
<td><span data-ttu-id="3a3af-597">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-597">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-598">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-598">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-599">Alle eksisterende reservationer, der påvirkes af antalsreguleringen i den kort plukkede lokation, fjernes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-599">All existing reservations that are affected by the quantity adjustment in the short-picked location are removed.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-600">En arbejdsundtagelse af typen <strong>Kort pluk</strong> er angivet, hvor <strong>Vareomfordeling</strong> = <strong>Manuel</strong>, <strong>Reguler lager</strong> = <strong>Ja</strong>, og <strong>Fjern reservationer</strong> = <strong>Nej/Ja</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-600">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No/Yes</strong>.</span></span> <span data-ttu-id="3a3af-601">Derudover er indstillingen <strong>Tillad manuel vareomfordeling</strong> aktiveret for arbejderen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-601">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="3a3af-602">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-602">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-603">Vælg menupunktet <strong>Kort pluk</strong> på lagerstedsappen, når du kører plukarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-603">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="3a3af-604">Angiv <strong>0</strong> i feltet <strong>Kort pluk antal</strong> (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-604">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="3a3af-605">I feltet <strong>Årsag</strong> skal du vælge <strong>Kort pluk med manuel omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-605">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
<li><span data-ttu-id="3a3af-606">Vælg lokationen/id'et på listen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-606">Select the location/license plate in the list.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="3a3af-607">Følgende handlinger finder sted i det aktuelle arbejde:</span><span class="sxs-lookup"><span data-stu-id="3a3af-607">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="3a3af-608">Pluklinjen er lukket, og det plukkede antal er 0 (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-608">The pick line is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="3a3af-609">Læg på lager-linjen annulleres.</span><span class="sxs-lookup"><span data-stu-id="3a3af-609">The put line is canceled.</span></span></li>
<li><span data-ttu-id="3a3af-610">Der oprettes en ny pluklinje.</span><span class="sxs-lookup"><span data-stu-id="3a3af-610">A new pick line is created.</span></span> <span data-ttu-id="3a3af-611">Den bruger den lokation/det ud, som brugeren har valgt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-611">It uses the location/license plate that the user selected.</span></span></li>
<li><span data-ttu-id="3a3af-612">Der oprettes en ny læg på lager-linje.</span><span class="sxs-lookup"><span data-stu-id="3a3af-612">A new put line is created.</span></span></li>
</ul>
</li>
<li><span data-ttu-id="3a3af-613">Der oprettes en lagertransaktion af typen <strong>Optælling</strong>, og afgangstypen <strong>Solgt</strong> oprettes for at repræsentere reguleringen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-613">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-614">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-614">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-615">En arbejdsundtagelse af typen <strong>Kort pluk</strong> er angivet, hvor <strong>Vareomfordeling</strong> = <strong>Manuel</strong>, <strong>Reguler lager</strong> = <strong>Ja</strong>, og <strong>Fjern reservationer</strong> = <strong>Nej</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-615">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No</strong>.</span></span> <span data-ttu-id="3a3af-616">Derudover er indstillingen <strong>Tillad manuel vareomfordeling</strong> aktiveret for arbejderen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-616">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="3a3af-617">Ingen</span><span class="sxs-lookup"><span data-stu-id="3a3af-617">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-618">Vælg menupunktet <strong>Kort pluk</strong> på lagerstedsappen, når du kører plukarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-618">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="3a3af-619">Angiv <strong>0</strong> i feltet <strong>Kort pluk antal</strong> (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-619">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="3a3af-620">I feltet <strong>Årsag</strong> skal du vælge <strong>Kort pluk med manuel omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-620">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-621">Handlingen kort pluk mislykkes, og der udløses en fejl.</span><span class="sxs-lookup"><span data-stu-id="3a3af-621">The short-picking action fails, and an error is thrown.</span></span></td>
<td><span data-ttu-id="3a3af-622">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-622">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-623">En arbejdsundtagelse af typen <strong>Kort pluk</strong> er angivet, hvor <strong>Vareomfordeling</strong> = <strong>Manuel</strong>, <strong>Reguler lager</strong> = <strong>Ja</strong>, og <strong>Fjern reservationer</strong> = <strong>Ja</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-623">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>Yes</strong>.</span></span> <span data-ttu-id="3a3af-624">Derudover er indstillingen <strong>Tillad manuel vareomfordeling</strong> aktiveret for arbejderen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-624">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="3a3af-625">Ingen</span><span class="sxs-lookup"><span data-stu-id="3a3af-625">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-626">Vælg menupunktet <strong>Kort pluk</strong> på lagerstedsappen, når du kører plukarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-626">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="3a3af-627">Angiv <strong>0</strong> i feltet <strong>Kort pluk antal</strong> (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-627">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="3a3af-628">I feltet <strong>Årsag</strong> skal du vælge <strong>Kort pluk med manuel omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-628">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
<li><span data-ttu-id="3a3af-629">Vælg lokationen/id'et på listen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-629">Select the location/license plate in the list.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="3a3af-630">Følgende handlinger finder sted i det aktuelle arbejde:</span><span class="sxs-lookup"><span data-stu-id="3a3af-630">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="3a3af-631">Pluklinjen er lukket, og det plukkede antal er 0 (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-631">The pick line is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="3a3af-632">Læg på lager-linjen annulleres.</span><span class="sxs-lookup"><span data-stu-id="3a3af-632">The put line is canceled.</span></span></li>
</ul>
</li>
<li><span data-ttu-id="3a3af-633">Der oprettes en lagertransaktion af typen <strong>Optælling</strong>, og afgangstypen <strong>Solgt</strong> oprettes for at repræsentere reguleringen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-633">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="3a3af-634">Alle eksisterende reservationer, der påvirkes af antalsreguleringen i den kort plukkede lokation/id, fjernes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-634">All existing reservations that are affected by the quantity adjustment in the short-picked location/license plate are removed.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-635">En arbejdsundtagelse af typen <strong>Kort pluk</strong> er angivet, hvor <strong>Vareomfordeling</strong> = <strong>Automatisk</strong>, <strong>Reguler lager</strong> = <strong>Ja/Nej</strong>, og <strong>Fjern reservationer</strong> = <strong>Ja/Nej</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-635">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Automatic</strong>, <strong>Adjust inventory</strong> = <strong>Yes/No</strong>, and <strong>Remove reservations</strong> = <strong>Yes/No</strong>.</span></span></td>
<td><span data-ttu-id="3a3af-636">Ikke relevant</span><span class="sxs-lookup"><span data-stu-id="3a3af-636">Not applicable</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-637">Vælg menupunktet <strong>Kort pluk</strong> på lagerstedsappen, når du kører plukarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-637">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="3a3af-638">Angiv <strong>0</strong> i feltet <strong>Kort pluk antal</strong> (nul).</span><span class="sxs-lookup"><span data-stu-id="3a3af-638">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="3a3af-639">I feltet <strong>Årsag</strong> skal du vælge <strong>Kort pluk med automatisk omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-639">In the <strong>Reason</strong> field, select <strong>Short Picking with automatic reallocation</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-640">Kort pluk, der involverer automatisk omfordeling, understøttes ikke.</span><span class="sxs-lookup"><span data-stu-id="3a3af-640">Short-picking that involves automatic reallocation isn't supported.</span></span></td>
<td><span data-ttu-id="3a3af-641">Kort pluk, der involverer automatisk omfordeling, understøttes ikke.</span><span class="sxs-lookup"><span data-stu-id="3a3af-641">Short-picking that involves automatic reallocation isn't supported.</span></span></td>
</tr>
</tbody>
</table>

#### <a name="change-the-inventory-status"></a><span data-ttu-id="3a3af-642">Skift status for lagerbeholdning</span><span class="sxs-lookup"><span data-stu-id="3a3af-642">Change the inventory status</span></span>

> [!NOTE]
> <span data-ttu-id="3a3af-643">Lagerstedshandlingen kan udføres fra flere indgangspunkter.</span><span class="sxs-lookup"><span data-stu-id="3a3af-643">This warehouse action can be performed from multiple entry points.</span></span> <span data-ttu-id="3a3af-644">Det eksempel, der vises her, bruger handlingen **Ændring af lagerstatus** på siden **Disponibel efter lokation**.</span><span class="sxs-lookup"><span data-stu-id="3a3af-644">The example that is shown here uses **Inventory status change** action on the **On-hand by location** page.</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="3a3af-645">Vigtig opsætningsparameter</span><span class="sxs-lookup"><span data-stu-id="3a3af-645">Key setup parameter</span></span></th>
<th><span data-ttu-id="3a3af-646">Batchantal er tilgængeligt</span><span class="sxs-lookup"><span data-stu-id="3a3af-646">Batch quantity is available</span></span></th>
<th><span data-ttu-id="3a3af-647">Vigtige brugertrin</span><span class="sxs-lookup"><span data-stu-id="3a3af-647">Key user steps</span></span></th>
<th><span data-ttu-id="3a3af-648">Lagerstedsarbejde</span><span class="sxs-lookup"><span data-stu-id="3a3af-648">Warehouse work</span></span></th>
<th><span data-ttu-id="3a3af-649">Ordrebekræftet batchreservation</span><span class="sxs-lookup"><span data-stu-id="3a3af-649">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="3a3af-650">Under fanen <strong>Lagersted</strong> i posten <strong>Lagersted</strong> er feltet <strong>Fjern reservationer og afmærkninger</strong> angivet til <strong>Reservationer</strong> eller <strong>Afmærkninger og reservationer</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-650">On the <strong>Warehouse</strong> tab, in the <strong>Warehouse</strong> record, the <strong>Remove reservations and markings</strong> field is set to <strong>Reservations</strong> or <strong>Markings and reservations</strong>.</span></span></td>
<td><span data-ttu-id="3a3af-651">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-651">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-652">Vælg en bestemt lokation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-652">Select a specific location.</span></span></li>
<li><span data-ttu-id="3a3af-653">Vælg en linje med en bestemt vare, lokation og id (hvis lokationen er id-styret).</span><span class="sxs-lookup"><span data-stu-id="3a3af-653">Select a line that has a specific item, location, and license plate (if the location is license plate–controlled).</span></span></li>
<li><span data-ttu-id="3a3af-654">Vælg <strong>Ændring af lagerstatus</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-654">Select <strong>Inventory status change</strong>.</span></span></li>
<li><span data-ttu-id="3a3af-655">Angiv feltet <strong>Lagerstatus</strong> til <strong>Blokering</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-655">Set the <strong>Inventory status</strong> field to <strong>Blocking</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-656">Ændringer i lagerstatus er ikke tilladt for antal, der er reserveret til arbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-656">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="3a3af-657">Antallet reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-657">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-658">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-658">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></li>
<li><span data-ttu-id="3a3af-659">To lagertransaktioner af typen <strong>Ændring af lagerstatus</strong> type oprettes for at repræsentere ændringen i lagerstatusdimensionen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-659">Two inventory transactions of the <strong>Inventory status change</strong> type are created to represent the change in the inventory status dimension.</span></span></li>
<li><span data-ttu-id="3a3af-660">Der oprettes en lagertransaktion af typen <strong>Lagerblokering</strong> og afgangstypen <strong>Reserveret fysisk</strong> oprettes for at repræsentere reservationen af det blokerede antal.</span><span class="sxs-lookup"><span data-stu-id="3a3af-660">An inventory transaction of the <strong>Inventory blocking</strong> type and the <strong>Reserved physical</strong> issue type is created to represent the reservation of the blocked quantity.</span></span></li>
</ul>
</td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-661">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-661">No</span></span></td>
<td><span data-ttu-id="3a3af-662">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-662">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-663">Ændringer i lagerstatus er ikke tilladt for antal, der er reserveret til arbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-663">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="3a3af-664">Reservationen fjernes.</span><span class="sxs-lookup"><span data-stu-id="3a3af-664">The reservation is removed.</span></span></li>
<li><span data-ttu-id="3a3af-665">To lagertransaktioner af typen <strong>Ændring af lagerstatus</strong> type oprettes for at repræsentere ændringen i lagerstatusdimensionen.</span><span class="sxs-lookup"><span data-stu-id="3a3af-665">Two inventory transactions of the <strong>Inventory status change</strong> type are created to represent the change in the inventory status dimension.</span></span></li>
<li><span data-ttu-id="3a3af-666">Der oprettes en lagertransaktion af typen <strong>Lagerblokering</strong> og afgangstypen <strong>Reserveret fysisk</strong> oprettes for at repræsentere reservationen af det blokerede antal.</span><span class="sxs-lookup"><span data-stu-id="3a3af-666">An inventory transaction of the <strong>Inventory blocking</strong> type and the <strong>Reserved physical</strong> issue type is created to represent the reservation of the blocked quantity.</span></span></li>
</ul>
</td>
</tr>
<tr>
<td rowspan='2'><span data-ttu-id="3a3af-667">Under fanen <strong>Lagersted</strong> i posten <strong>Lagersted</strong> er feltet <strong>Fjern reservationer og afmærkninger</strong> angivet til <strong>Ingen</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-667">On the <strong>Warehouse</strong> tab, in the <strong>Warehouse</strong> record, the <strong>Remove reservations and markings</strong> field is set to <strong>None</strong>.</span></span></td>
<td><span data-ttu-id="3a3af-668">Ja</span><span class="sxs-lookup"><span data-stu-id="3a3af-668">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="3a3af-669">Vælg en bestemt lokation.</span><span class="sxs-lookup"><span data-stu-id="3a3af-669">Select a specific location.</span></span></li>
<li><span data-ttu-id="3a3af-670">Vælg en linje med en bestemt vare, lokation og id (hvis lokationen er id-styret).</span><span class="sxs-lookup"><span data-stu-id="3a3af-670">Select a line that has a specific item, location, and license plate (if the location is license plate–controlled).</span></span></li>
<li><span data-ttu-id="3a3af-671">Vælg <strong>Ændring af lagerstatus</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-671">Select <strong>Inventory status change</strong>.</span></span></li>
<li><span data-ttu-id="3a3af-672">Angiv feltet <strong>Lagerstatus</strong> til <strong>Blokering</strong>.</span><span class="sxs-lookup"><span data-stu-id="3a3af-672">Set the <strong>Inventory status</strong> field to <strong>Blocking</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="3a3af-673">Ændringer i lagerstatus er ikke tilladt for antal, der er reserveret til arbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-673">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td><span data-ttu-id="3a3af-674">Antallet reserveres igen for samme batch.</span><span class="sxs-lookup"><span data-stu-id="3a3af-674">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="3a3af-675">Systemet tildeler tilfældigt en lokation og et id (hvis lokationen er id-styret), hvor antallet er tilgængeligt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-675">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="3a3af-676">Nr.</span><span class="sxs-lookup"><span data-stu-id="3a3af-676">No</span></span></td>
<td><span data-ttu-id="3a3af-677">Se den forrige række.</span><span class="sxs-lookup"><span data-stu-id="3a3af-677">See the previous row.</span></span></td>
<td><span data-ttu-id="3a3af-678">Ændringer i lagerstatus er ikke tilladt for antal, der er reserveret til arbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-678">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td><span data-ttu-id="3a3af-679">Ændringer i lagerstatus er ikke tilladt.</span><span class="sxs-lookup"><span data-stu-id="3a3af-679">Inventory status changes aren't allowed.</span></span></td>
</tr>
</tbody>
</table>

## <a name="limitations"></a><span data-ttu-id="3a3af-680">Begrænsninger</span><span class="sxs-lookup"><span data-stu-id="3a3af-680">Limitations</span></span>

- <span data-ttu-id="3a3af-681">Reservationsfunktionaliteten for fleksibel dimension for lagerstedsniveau understøtter ikke følgende funktioner:</span><span class="sxs-lookup"><span data-stu-id="3a3af-681">The flexible warehouse-level dimension reservation functionality doesn't support the following features:</span></span>

    - <span data-ttu-id="3a3af-682">Styring af fastvægt</span><span class="sxs-lookup"><span data-stu-id="3a3af-682">Catch weight management</span></span>
    - <span data-ttu-id="3a3af-683">Fysisk negativt lager</span><span class="sxs-lookup"><span data-stu-id="3a3af-683">Physical negative inventory</span></span>
    - <span data-ttu-id="3a3af-684">Reservation i forhold til bestilt forsyning</span><span class="sxs-lookup"><span data-stu-id="3a3af-684">Reservation against ordered supply</span></span>
    - <span data-ttu-id="3a3af-685">Flytteordrer og pluk af råvarer</span><span class="sxs-lookup"><span data-stu-id="3a3af-685">Transfer orders and raw material picking</span></span>

- <span data-ttu-id="3a3af-686">Der er begrænsninger for containerkonsolideringsreglen for pakning efter vejledningsenhed.</span><span class="sxs-lookup"><span data-stu-id="3a3af-686">The container consolidation rule for packing by directive unit has limitations.</span></span> <span data-ttu-id="3a3af-687">I forbindelse med ordrebekræftede reservationer anbefales det, at du ikke bruger skabeloner til containerbuild, hvor feltet **Pakning efter vejledningsenhed** er aktiveret.</span><span class="sxs-lookup"><span data-stu-id="3a3af-687">For order-committed reservations, we recommend that you not use container build templates where the **Pack by directive unit** field is enabled.</span></span> <span data-ttu-id="3a3af-688">I det aktuelle design bruges lokationsvejledninger ikke, når der oprettes lagerstedsarbejde.</span><span class="sxs-lookup"><span data-stu-id="3a3af-688">In the current design, location directives aren't used when warehouse work is created.</span></span> <span data-ttu-id="3a3af-689">Derfor er det kun den laveste enhed i enhedsseriegruppen (lagerenheden), der anvendes under containeriseringsbølgetrinnet.</span><span class="sxs-lookup"><span data-stu-id="3a3af-689">Therefore, only the lowest unit in the unit sequence group (the inventory unit) is applied during the containerization wave step.</span></span>
