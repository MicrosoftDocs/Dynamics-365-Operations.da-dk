...
Nr. Hvis du redigerer Talent-programbrugeren, vil integrationen mellem Human Resources og Common Data Service muligvis ikke fungere. Følgende tabel viser standardindstillingerne for Talent-programbrugeren.
...
Som standard er Common Data Service-integrationen slået fra i nye miljøer, der ikke indeholder de leverede demonstrationsdata. Som standard er den aktiveret i nye miljøer, der indeholder demonstrationsdata, og datasynkroniseringen starter, når miljøet klargøres. Når miljøet er klar til at synkronisere data, kan du aktivere integrationen. Du kan finde flere oplysninger under [Konfigurere Common Data Service-dataintegration](hr-admin-integration-common-data-service.md).
...
## <a name="can-i-map-financial-dimensions-to-transfer-between-human-resources-and-finance"></a>Kan jeg tilknytte økonomiske dimensioner for at overføre mellem Personale og Finance?
...
For data i Finance and Operations, som ikke findes i Personale, kan du sammenkæde de to systemer ved hjælp af **Konfigurer links** i Personale.
...
Denne fejl kan opstå, hvis medarbejderne ikke har en aktiv post med ansættelsesoplysninger i Personale. Du kan løse dette ved at gå til **Personalestyring \> Medarbejdere \> Ansættelseshistorik \> Datoadministrator** og kontrollere, at der findes en aktiv post med ansættelsesoplysninger.
...
Felter kan tilføjes eller fjernes fra integrationsopgaven. Ikke alle datafelter, der findes på Common Data Service-enheden, udfyldes fra Personale.
...
## <a name="i-set-up-integration-as-a-batch-job-but-human-resources-lost-connection-to-the-destination-system-how-can-i-send-the-same-set-of-changes-to-the-destination-system"></a>Jeg konfigurerede integration som et batchjob, men så mistede Personale forbindelsen til destinationssystemet. Hvordan kan jeg sende det samme sæt ændringer til destinationssystemet?
...
Nej, integration er i øjeblikket envejs (Personale til Finance and Operations). Der er dog en tilgængelig standardskabelon, som kan bruges til at sende data fra Personale til Finance.
...
## <a name="i-just-created-new-finance-and-human-resources-environments-and-im-getting-the-error-the-data-value-violates-integrity-constraints-why"></a>Jeg har netop oprettet nye Finance- og Personale-miljøer, og jeg får fejlmeddelelsen "Dataværdien overtræder integritetsbegrænsningerne". Hvorfor?
...
## <a name="after-integrating-human-resources-and-finance-i-dont-see-my-human-resources-data-in-finance-what-do-i-do"></a>Når jeg integrerer Personale og Finance, kan jeg ikke se mine Personale-data i Finance. Hvad skal jeg gøre?

Integrationen i Finance er en totrinsproces. Kontroller først, at Personale-dataene er opdaterede og tilgængelige i Common Data Service. Dette er en synkronisering i nær-realtid, som du kan kontrollere i Power Apps ved at se på dataene i dataenhederne.
...
Nummerserien for **Lokations-id** bruger det samme mønster i både Personale og Finance. Nummerserien skal være unik på begge sider, så der ikke opstår adressekollisioner under integrationen af data fra Common Data Service til Finance and Operations.

Ved implementering af Personale skal du kontrollere, at nummerserierne ikke de samme i Personale og Finance. Kontroller, at alle nummerserier ikke er ens, hvor data kan vedligeholdes i begge systemer.
...
Kontroller, at det er de korrekte juridiske enheder, du opretter tilknytning til. Synkroniseringen af en juridisk enhed er ikke en del af standardskabelonen, så det forventes, at de enkelte juridiske enheder, der findes i Personale og Common Data Service, også findes i Finance.
...
- Dataintegrator (DI): 
  ...
  - [Svar på DSR-anmodninger om systemgenererede logfiler i Power Apps, Microsoft Power Automate og Common Data Service](https://docs.microsoft.com/powerapps/administrator/powerapps-gdpr-dsr-guide-systemlogs)
