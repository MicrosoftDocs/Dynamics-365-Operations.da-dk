---
title: Måneder i året
description: Dette emne beskriver den indstilling Måneder i året, der er angivet for Dynamics 365 Human Resources.
author: jaredha
ms.date: 02/05/2021
ms.topic: article
ms.prod: ''
ms.technology: ''
audience: Application User
ms.custom: ''
ms.assetid: ''
ms.search.region: Global
ms.author: jaredha
ms.search.validFrom: 2021-02-05
ms.dyn365.ops.version: Human Resources
ms.openlocfilehash: 34b7489593d78ad0fd779b58b3a15e63b0dd4d39
ms.sourcegitcommit: 879ee8a10e6158885795dce4b3db5077540eec41
ms.translationtype: HT
ms.contentlocale: da-DK
ms.lasthandoff: 05/18/2021
ms.locfileid: "6053821"
---
# <a name="months-of-year"></a>Måneder i året

[!include [Applies to Human Resources](../includes/applies-to-hr.md)]

Dette emne beskriver den indstilling Måneder i året, der er angivet for Dynamics 365 Human Resources.

Fysisk navn: mshr_monthsofyear

Denne fasttekst indeholder den indstilling, der er angivet måneder i året.

| Værdi | Mærkat | Beskrivelse |
| --- | --- | --- |
| 200000000 | None | Der er ikke valgt en værdi. |
| 200000001 | Januar | Januar. |
| 200000002 | Februar | Februar. |
| 200000003 | Marts | Marts. |
| 200000004 | April | April. |
| 200000005 | Maj | Maj. |
| 200000006 | Juni | Juni. |
| 200000007 | Juli | Juli. |
| 200000008 | August | August. |
| 200000009 | September | September. |
| 200000010 | Oktober | Oktober. |
| 200000011 | November | November. |
| 200000012 | December | December. |

## <a name="see-also"></a>Se også

[Introduktion til API-integration for ansøgersporingssystem](hr-admin-integration-ats-api-introduction.md)<br>
[Eksempelforespørgsel til ansøger til ansættelse](hr-admin-integration-ats-api-candidate-to-hire-example-query.md)


[!INCLUDE[footer-include](../includes/footer-banner.md)]