...
-   Kolonnedefinitionens overskriftsområde indeholder den overskrift og formatering, der vises i rapporten. Et sidehoved kan gælde for en enkelt kolonne med data, kan strække sig over flere kolonner eller kan anvende kolonner på en betinget grundlag. Kolonnedefinitionen kan indeholde så mange hovedet kolonnerækker som du har brug for. **Bemærk:** kolonneoverskrifter gælder for hver datakolonne i rapporten. Rapportoverskrifter gælder for hele rapporten. Du kan definere rapportoverskrifter under fanen **Sidehoveder og sidefødder** i rapportdefinitionen.
...
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | Undertrykke understregning for en kolonne, hvor kommandoen understregningstegn (**---**) eller en kommando til dobbelt understregning (**===**) er angivet i rækkedefinitionen. Du kan for eksempel ikke understrege beløb, der er produceret ved en procentberegning.                                                                        |
...
Rapportdesigner kan generere kolonnehoveder automatisk ud fra autotekstkoder. Autotekstkoder er variabler, der opdateres, hver gang der genereres en rapport. Alle kolonneoverskrifter kan indeholde disse koder for at angive rapportoplysninger, som kan variere, f.eks datoer eller perioder tal. Du kan derfor bruge en kolonnedefinitionen til flere rapportdefinitioner, tidsperioder og rapporteringstræer. Da autotekstkoder afhænger af kalenderoplysningerne fra detaljerækkerne i kolonnedefinitionen, understøttes de kun for kolonnerne **CALC**, **FD** og **WKS**. Den måde, en autotekstkode vises på i kolonneoverskriftscellen, påvirker, hvordan oplysningerne vises i rapporten. I dialogboksen **kolonneoverskrift** vises autotekstkoder i forskellige situationer. Derfor vises teksten i forskellige situationer i rapporten. For eksempel i et standard kalenderår, **@CalMonthLong**løser måned **7** til **juli**. Hvis navnet på måneden skal skrives med store bogstaver (for eksempel **JULI**), Angiv koden Autotekst i store bogstaver i den **kolonne sidehovedtekst** felt. F.eks.**@CALMONTHLONG**. Du kan blande koder og tekst. For eksempel skal du indtaste følgende sidehovedtekst: **periode @FiscalPeriod-@FiscalYearfra @StartDatetil@EndDate**. Den rapportoverskrift, der genereres, ligner følgende tekst: **Periode 1-02 fra 01-01-02 til 01-31-02**. **Bemærk:** formatet på noget af teksten, som lang dato, afhænger af indstillingerne under International i Dynamics-365 for operationer server. Hvis du vil ændre disse indstillinger, skal du klikke på knappen **Start**, klikke på **Kontrolpanel** og derefter klikke på **Område og sprog**. I følgende tabel vises de tilgængelige indstillinger for autotekst til kolonneoverskrifter.
...
| Periodenummeret (@FiscalPeriod)           | Udskriv det numeriske format for den regnskabsperiode, der er angivet for den pågældende kolonne. Hvis kolonnen strækker sig over flere perioder, udskrives den sidste periode i intervallet.                                                                                                                                   |
| Beskrivelse (@FiscalPeriodName)  | Udskriv den beskrivelse af regnskabsperioden, der er angivet i de økonomiske data.                                                                                                                                                                                                                    |
...
| Enhedsnavnet fra træet (@UnitName)         | Hvis du begrænser en kolonne til en bestemt enhed af rapporteringstræet, kan du udskrive enhedsnavnet i kolonneoverskriften.                                                                                                                                                                                     |
...
| Bogen kode (@BookCode)                   | Udskriv den bogkode, der er angivet i kolonnen.                                                                                                                                                                                                                                             |
...
-   Hvis du angiver kolonneoverskrifter, der overlapper hinanden, overlapper de, når de udskrives på rapporten. Rapporten er genereret, men følgende advarsel vises i den **rapport kø Status** felt: "ved hjælp af Base kolonneoverskrifterne overlapper andre kolonneoverskrifter og kan forårsage overlappende tekst". For eksempel hovedet definitionen på kolonne B er B til grundlæggende + 1, og definitionen af overskrift i kolonne D er grundlæggende + 1 F. Brevhoveder i dette tilfælde udskrives oven på hinanden og er ulæselige. Når BASE bruges i en definition af typen **Opslag fra/opslag til**, skal du sørge for at få vist den rapport, der er genereret, for at se, om overskrifterne overlapper hinanden.
...
-   For betinget udskrivning kolonner (**P&lt;B**, **P&gt;= B**), betinget spanning sidehoveder opfører sig som en regelmæssig kolonnedefinitionen for hovedet. Hvis betingelsen for eksempel er falsk, starter alle efterfølgende kolonner, der matcher udbredelsesbetingelsen, udskrivningen af overskriften.
...
3.  Vælg en kolonnetype på listen. Følgende tabel beskriver de forskellige kolonnetyper.
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>Hvis dit regnskabssystem understøtter attributter, vises en konto- eller transaktionsattribut i kolonnen. En attribut, som skal gælde for en enkelt fuld konto, udtrækker underliggende konto- eller transaktionsoplysninger fra de økonomiske data. Kontoniveau attributter vises data fra kontoen, og transaktionsniveau attributter, der viser data, der fandt sted på det tidspunkt, hvor transaktionen blev bogført. Hvis du vælger <strong>ATTR</strong> som kolonnetype, skal du angive kategorien attribut i den <strong>bog kode eller attributten kategori</strong> detaljer række kolonnedefinitionen.</td>
    </tr>
    </tbody>
    </table>
...
| BASE +\# | Brug det år, der er \#år efter det basisår. Hvis du for eksempel vil bruge det tredje år efter basisåret, skal du skrive **BASE+ 3**. |
| BASE-\# | Brug det år, der er \#år før det basisår. Hvis du vil bruge det foregående år, skal du skrive **BASE- 1**.                 |
...
| BASE -\#: grundlæggende +\# | Brug flere perioder fra flere perioder før basisperioden til flere perioder efter basisperioden. Hvis du for eksempel vil bruge de tre foregående perioder, basisperioden og de to efterfølgende perioder, skal du skrive **BASE-3:BASE+ 2**. |
...
Du kan gå ud over regnskabsårets grænser i en periodespecifikation, og du kan blande år i et interval af perioder. Du for eksempel angive perioder som **BASE-5** (til at repræsentere seneste seks perioder) og køre en rapport, der er en grundlæggende periode 2. I dette tilfælde viser rapporten data for de to første perioder i det angivne regnskabsår og de sidste fire perioder i forrige regnskabsår.
...
3.  På formellinjen over navigationsruden eller i cellen **Periode** skal du udfylde formlen. Erstatte et nummertegn (\#) med den relevante værdi.
...
Et dimensionsfilter bruges til at begrænse **FD**-kolonnen til bestemte dimensionsværdier. Filteret kan indeholde en enkelt dimension, et interval af dimensioner eller en gruppe af dimensioner. Filteret kan også indeholde dimensionsværdisæt. Da dimensionsværdier kan variere, en... \financial-dimensions\dimension-based systemet behøver ikke at svare til en bestemt længde. Filteret anvendes, uanset om rapporten indeholder et rapporteringstræ. Du kan bruge jokertegn (\* eller?) i en hvilken som helst placering. Når du angiver flere konti, kan du placere et komma mellem firmaer, som i følgende eksempel: + konto =\[1200\], + konto =\[1100\], afdeling =\[01?\] Hvis du vil modtage alle afdelinger for en bestemt konto, kan du udelukke dimensionen Afdeling fra dimensionsfilteret. De to følgende dimensionsfiltre håndteres for eksempel på samme måde:

-   + Højde =\[1100\], afdeling
-   + Højde =\[1100\]

Du kan også bruge en kombination af alfanumeriske tegn for at opnå et nøjagtigt match, og du kan definere delvise dimensioner. For eksempel **placering = \[10\*\]** omfatter alle placering dimensionsværdier, der begynder med 10.
...
**Bemærk:** De indstillinger, der er beskrevet her, kan variere afhængigt af ERP-systemet. Yderligere oplysninger finder du din [dokumentationen til Microsoft ERP-system](https://www.microsoft.com/en-us/download/details.aspx?id=5916).
